...
Sequencing results for these samples differed significantly (p < 0.001 from permutational ANOVA) from samples preserved using the Xpedition^TM^ lysis buffer (Supplementary Figures \@ref(fig:buffer-compare-long) and \@ref(fig:buffer-compare-short)); as such these samples were excluded from our spatial analyses.
...
buffer_asv_table <- phyloseq::prune_samples(buffer_asv_table, samples = samples)

buffer_asv_bc_dist <- buffer_asv_table %>%
...
```{r}
buffer_asv_dbrda <- vegan::dbrda(
  buffer_asv_bc_dist ~ 1,
  data = as_tibble(phyloseq::sample_data(buffer_asv_table))
)
plot(buffer_asv_dbrda, type = "none")
points(buffer_asv_dbrda, select = with(as_tibble(phyloseq::sample_data(buffer_asv_table)),
...
points(buffer_asv_dbrda, select = with(as_tibble(phyloseq::sample_data(buffer_asv_table)),
       ...
       col = "red", pch = 3)
legend("bottomright", legend = c("Xpedition 2015", "Xpedition 2016", "LifeGuard 2016"), col = c("green", "blue", "red"), pch = 1:3)
legend("topright", legend = with(as_tibble(phyloseq::sample_data(buffer_asv_table)), paste(tech, amplicon)) %>% unique() %>% sort(), lty = 1:3)
vegan::ordiellipse(buffer_asv_dbrda, groups = with(as_tibble(phyloseq::sample_data(buffer_asv_table)), paste(tech, amplicon)),
                   lty = 1:3)
```

```{r}
tech_asv_table <- rdd(proto_physeq) %>%
  phyloseq::subset_samples(tech != "Ion Torrent") %>%
...
samples <- phyloseq::sample_data(tech_class_table) %>%
  as_tibble(rownames = "sample") %>%
...
tech_class_table <-
  phyloseq::prune_samples(tech_class_table, samples = samples) %>%
  phyloseq::transform_sample_counts(function(x) x / sum(x)) %>%
  phyloseq::prune_taxa(taxa = (phyloseq::otu_table(tech_class_table) %>% t() %>% as.data.frame() %>% do.call(what = pmax)) > 0.01)
...
(ref:ppcoa) Only within-sample variation is displayed; variation between samples (site, position, year) have been partialed out.
