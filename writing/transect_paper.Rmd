...
Independent error models were fit for each sequencing run (i.e., long *vs.* short amplicons).
Chimeras within each region were removed using `removeBimeraDenovoTable` from DADA2.

The ITS2 region was used to identify full-length amplicons ASVs, due to the fact that it was the only full-length region/domain shared by both long and short amplicons.
