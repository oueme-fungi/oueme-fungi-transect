...
(ref:ppcoa) Spatio-temporal variation (i.e., variation between different soil samples) has been partialled out.
Only samples with at least 100 reads for all three approaches are included.
ASVs are clustered by taxonomic class.
Abbreviations represent the approximate direction of increase for the classes with the highest scores: `r glue::glue_data(tech_class_dbrda_scores, "**{abbrev}**: {class}") %>% glue::glue_collapse(sep = "; ", last = "; and ")`.


```{r ppcoa, fig.cap = "(ref:ppcoa-short). (ref:ppcoa)", fig.scap = "(ref:ppcoa-short)"}
plot(tech_class_dbrda, type = "none")
points(tech_class_dbrda, select = with(as_tibble(phyloseq::sample_data(tech_class_table)),
...
points(tech_class_dbrda, select = with(as_tibble(phyloseq::sample_data(tech_class_table)),
       ...
       col = "red", pch = 3)
text(
  x = tech_class_dbrda_scores$MDS1,
  y = tech_class_dbrda_scores$MDS2,
  labels = tech_class_dbrda_scores$abbrev
  )
legend("bottomleft", legend = c("Illumina Short", "PacBio Short", "PacBio Long"), col = c("red", "green", "blue"), pch = c(3, 1, 2))
```
...
vegan::ordispider(
  tech_class_nmds,
  groups = with(
    as_tibble(phyloseq::sample_data(tech_class_table)),
...
points(tech_class_nmds, select = with(as_tibble(phyloseq::sample_data(tech_class_table)),
                                     tech == "PacBio" & amplicon == "Short" ),
       col = "green", pch = 1)
points(tech_class_nmds, select = with(as_tibble(phyloseq::sample_data(tech_class_table)),
                                     tech == "PacBio" & amplicon == "Long"),
...
(ref:buffer-pcoa-short) Unconstrained Principal Coordinate Analysis ordination of samples preserved with different buffers
