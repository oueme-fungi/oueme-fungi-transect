...
vennplot_ASV <- venn_ASV %>%
  ...
  mutate(set = map_chr(set,
                     ...
                     paste(collapse = "&"))) %>%
  mutate(
    reads_frac = paste(frac_pb_500, frac_pb_483, frac_is_057, `frac_SH-2257`,
                       sep = "/") %>%
      gsub("/+", "/", .) %>%
      gsub("(^/|/$)", "", .),
...
eulerr::venn(deframe(select(vennplot_ASV, set, ASVs)) %>% replace_na(0)) %>%
  plot(
    legend = list(
...
```{r ECM-families, fig.cap = "(ref:ecm-families)", include = FALSE}
  readd(taxon_reads, cache = cache) %>%
  taxon_plot(
    family,
...
light_plan %>%
  ...
  xlab("Distance (m)") +
  ggnomics::facet_nested(metric + tech + amplicon ~ guild, scales = "fixed") +
...
variog_empir %>%
  ...
  ggnomics::facet_nested(metric + tech + amplicon ~ guild, scales = "free")
...
variog_data %>%
  ...
  unnest(variofitST) %>%
  arrange(metric, amplicon, tech, guild) %>%
  select(Guild = guild, Amplicon = amplicon, Tech = tech, Metric = metric,
  ...
  select(Guild, Amplicon, Tech, Metric, "Space Range" = Range, "Time Range") %>%
...
full_ecdf <- readd(reads_table, cache = cache) %>%
  dplyr::filter(region  %in% c("long", "short")) %>%
  dplyr::group_by(seq_run) %>%
  ...
  scale_color_brewer(type = "qual",
                     labels = c("Short - Ion", "Short - PacBio", "Long - PacBio", "Short - Illumina"),
                     name = NULL, palette = "Set2") +
  scale_y_continuous(name = "Cumulative read fraction") +
  scale_x_continuous(name = "Length (bp)") +
  theme(legend.position = c(1, 0.02), legend.justification = c(1, 0), legend.background = element_blank())
...
readd(region_table, cache = cache) %>%
...
its2_dens <- readd(reads_table, cache = cache) %>%
  dplyr::left_join(dplyr::select(datasets, seq_run, tech)) %>%
...
its2_ecdf <- readd(reads_table, cache = cache) %>%
  dplyr::filter(region == "ITS2") %>%
  dplyr::group_by(seq_run) %>%
  dplyr::mutate(reads = reads / sum(reads)) %>%
  ggplot(aes(length, weight = reads, color = seq_run, group = seq_run)) +
...
```{r short-its, fig.cap = ""}
readd(big_seq_table_ITS2, cache = cache) %>%
  colSums() %>%
  enframe(name = "ITS2", value = "nreads") %>%
  mutate(length = nchar(ITS2)) %>%
  filter(length <= 140) %>%
  mutate_at("ITS2", ~tzara::seqhash(chartr("T", "U", .))) %>%
  semi_join(readd(taxon_reads, cache = cache), ., by = c("label" = "ITS2")) %>%
  # group_by(ITS2) %>%
  # filter(!"Short" %in% amplicon) %>%
  taxon_plot(kingdom, y = reads)
```

(ref:sample-depth) Sequencing depth along transects at the two sites Ang and Gan for the years 2015 and 2016, after full bioinformatics pipeline.
Ang 2015 samples are underrepresented, especially in the PacBio long amplicon dataset.
Dotted horizontal line at proposed cutoff for inclusion in community analysis.
"2016 X" is using the newer batch of extraction buffer, "2016" is using the older batch.

```{r sample-depth, fig.cap = "(ref:sample-depth)"}
readd(proto_physeq, cache = cache) %>% {
  mutate(
    sample_data(.),
    reads = otu_table(.) %>% rowSums()
  )
} %>%
  filter(tech != "Ion Torrent", !is.na(year), !is.na(seq_run), !is.na(x)) %>%
  complete(nesting(seq_run, tech, amplicon), site, x, nesting(year, qual)) %>%
  mutate(
    qual = fct_collapse(qual, " " = c("*", "")),
    "Sample group" = paste(year, qual)
  ) %>%
  ggplot(aes(x = x, y = reads, group = `Sample group`, fill = `Sample group`)) +
  geom_col(position = "dodge") +
  ggnomics::facet_nested(
    cols = vars(tech, amplicon, site),
    rows = vars(`Sample group`),
    nest_line = TRUE,
    resect = unit(1, "mm")
  ) +
  scale_y_log10() +
...
(ref:plate-depth) Sequencing depth for each well in the sequencing plates (001, 002), after full bioinformatics pipeline.
Darker shading represents greater sequencing depth, as a fraction of the total number of sequences from the sequencing run.
The border color for each well where sequences were recovered indicates the sample group, i.e. site name (Ang or Gan), sampling year (2015 or 2016), and buffer batch (presence or absence of X; 2016 samples with X are the good buffer); Pos = positive control, Blank = PCR blank.
Ion Torrent (is_057) samples for the two plates are mixed, shown here as plate 001.
Note that many of the Ang2015 samples are missing or have extremely low number of reads for the pb_500 (long PacBio) sequencing run.

```{r plate-depth, fig.cap = "(ref:plate-depth)"}
readd(proto_physeq, cache = cache) %>% {
  mutate(
    sample_data(.),
    reads = otu_table(.) %>% rowSums()
  ...
  mutate(
    reads = reads/sum(reads),
    row = factor(LETTERS[row], levels = rev(LETTERS[1:8])),
    qual = fct_collapse(qual, " " = c("*", "")),
    "Sample group" = coalesce(
      str_c(site, year, qual),
      as.character(sample_type),
      "Unused"
      )
  ) %>%
  ggplot(aes(x = column, y = row, fill = reads, color = `Sample group`)) +
  geom_tile(size = 0.4, height = 0.9, width = 0.9) +
  ggnomics::facet_nested(cols = vars(plate), rows = vars(tech, amplicon),
                         resect = unit(1, "mm"), nest_line = TRUE) +
...
(ref:raw-depth) Sequencing depth for each well in the sequencing plates (001, 002), after demultiplexing and primer trimming.
Darker shading represents greater sequencing depth, as a fraction of the total number of sequences from the sequencing run.
The border color for each well where sequences were recovered indicates the sample group, i.e. site name (Ang or Gan), sampling year (2015 or 2016), and buffer batch (presence or absence of X; 2016 samples with X are the good buffer); Pos = positive control, Blank = PCR blank, NA = unused well.
Ion Torrent (is_057) samples for the two plates are mixed, shown here as plate 001.
Note that many of the Ang2015 samples are missing or have extremely low number of reads for the pb_500 (long PacBio) sequencing run.

```{r raw-depth, fig.cap = "(ref:raw-depth)"}
readd(qstats_n, cache = cache) %>%
  mutate(file = str_replace(file, ".*/", "") %>%
           str_replace("[.].*", "")) %>%
  tidyr::extract(col = "file", into = c("seq_run", "plate", "row", "col"),
                 regex = "([[:alpha:]]{2}[-_]\\d+)_(\\d+)-([A-H])(\\d+)") %>%
  filter(is.na(step), !is.na(seq_run)) %>%
  mutate_at("col", as.integer) %>%
  left_join(
    readd(proto_physeq, cache = cache) %>%
      sample_data() %>%
      select(plate, row, column, site, year, qual, sample_type) %>%
      mutate(row = LETTERS[row]) %>%
      unique(),
    by = c("plate", "row", "col" = "column")
  ) %>%
  left_join(select(datasets, seq_run, tech, amplicon), by = "seq_run") %>%
  group_by(seq_run, plate) %>%
  mutate(
    nreads = nreads/sum(nreads),
  ...
  ggplot(aes(x = col, y = row, fill = nreads, color = `Sample group`)) +
  geom_tile(size = 0.4, height = 0.9, width = 0.9) +
  ggnomics::facet_nested(cols = vars(plate), rows = vars(tech, amplicon),
                         resect = unit(1, "mm"), nest_line = TRUE) +
  scale_fill_gradient(low = "gray80", high = "black") +
...
(ref:pcr-concentration) Concentration of PCR products in ng/µL for the two plates of samples.

```{r pcr-concentration, fig.cap = "(ref:pcr-conentration)"}
tidyxl::xlsx_cells(here("config/Brendan_soil3.xlsx"), sheets = c("BSoil 1 Pico green", "BSoil 2 pico green", "BSoil 1 Picco Green ITS 1 LR5", "BSoil 2 Picco Green its1 LR5")) %>%
  filter(between(col, 4, 12), between(row, 77, 84), numeric > 0) %>% 
  mutate(row = LETTERS[row - 76], col = col - 3,
         sheet = plyr::mapvalues(sheet, c("BSoil 1 Pico green", "BSoil 2 pico green", "BSoil 1 Picco Green ITS 1 LR5", "BSoil 2 Picco Green its1 LR5"), c("short_001", "short_002", "long_001", "long_002"))) %>%
  select(sheet, row, col, Concentration = numeric) %>%
  separate(sheet, into = c("amplicon", "plate")) %>%
  left_join(
    readd(proto_physeq, cache = cache) %>%
      sample_data() %>%
      select(plate, row, column, site, year, qual, sample_type) %>%
  ...
  ggplot(aes(x = col, y = row, fill = Concentration, color = `Sample group`)) +
  geom_tile(size = 0.4, height = 0.9, width = 0.9) +
  ggnomics::facet_nested(cols = vars(plate), rows = vars(amplicon),
                         resect = unit(1, "mm"), nest_line = TRUE) +
  scale_fill_gradient(low = "gray80", high = "black") +
  coord_equal() +
  scale_color_discrete(name = "Sample group") +
...
(ref:dna-conc) DNA concentration in soil extracts.

```{r dna-conc, fig.cap = "(ref:dna-conc)"}
readxl::read_xls(here("config/Brendan soil samples DNA quantification.xls"),
                 col_types = c("text", "text", "numeric")) %>%
  filter(str_detect(Sample, "(T\\d-S\\d+|V\\dP\\d T\\d+)")) %>%
  rename(year = Year) %>%
  ...
  left_join(
    readd(proto_physeq, cache = cache) %>%
      sample_data() %>%
      select(plate, row, column, site, year, qual, sample_type, x) %>%
      mutate(row = LETTERS[row]) %>%
      unique(),
    by = c("site", "year", "qual", "x")
  ) %>%
  mutate(
    row = factor(row, levels = rev(LETTERS[1:8])),
    qual = ifelse(qual == "*", "", qual),
    "Sample group" = coalesce(
      str_c(site, year, qual),
      as.character(sample_type)
      )
  ) %>%
  ggplot(aes(x = column, y = row, fill = `Conc. (ng/µl)`, color = `Sample group`)) +
  geom_tile(size = 0.4, height = 0.9, width = 0.9) +
  ggnomics::facet_nested(cols = vars(plate),
                         resect = unit(1, "mm"), nest_line = TRUE) +
  scale_fill_gradient(low = "gray80", high = "black", na.value = "white") +
  coord_equal() +
  scale_color_discrete(name = "Sample group") +
  scale_x_continuous(breaks = 1:9) +
  theme(strip.background = element_blank())
...
```{r}
physeq <- readd(proto_physeq, cache = cache) %>%
  `sample_data<-`(inset2(
    sample_data(.),
    "qual",
  ...
  merge_samples(group = glue::glue_data(
    sample_data(.),
    "{year}_{qual}_{tech}_{amplicon}"
  ))

ranks <- c("root", "kingdom", "phylum", "class", "order", "family", "genus")
physeq <- readd(taxon_reads, cache = cache) %>%
  mutate(root = "Root") %>%
  group_by(label) %>%
...
taxmap$data$read_diff <- metacoder::compare_groups(taxmap, "read_count", cols = c("2016_-_PacBio_long", "2016_X_PacBio_long"), groups = c("Xpedition", "Lifeguard"))
```
...
taxa::filter_taxa(
  taxmap,
  taxmap$data$read_count$`2016_-_PacBio_long` > 0 |
    taxmap$data$read_count$`2016_X_PacBio_long` > 0,
  drop_obs = TRUE,
  reassign_obs = FALSE
  ) %>%
metacoder::heat_tree(
  ...
  node_color_range = metacoder::diverging_palette(),
  edge_size_interval = c(0, 0.3),
  output_file = here("temp/buffer.pdf")
)
...
taxmap$data$read_diff <- metacoder::compare_groups(
  taxmap,
  "read_count",
  cols = c("2015_-_PacBio_long", "2016_X_PacBio_long", "2016_-_PacBio_long"),
  groups = c("Xpedition 2015", "Xpedition 2016", "Lifeguard 2016")
)
taxa::filter_taxa(
  taxmap,
  taxmap$data$read_count$`2015_-_PacBio_long` > 0 |
    taxmap$data$read_count$`2016_X_PacBio_long` > 0 |
    taxmap$data$read_count$`2016_-_PacBio_long` > 0,
  drop_obs = TRUE,
  reassign_obs = FALSE
...
taxmap$data$read_diff <- metacoder::compare_groups(
  taxmap,
  "read_count",
  cols = c("2015_-_PacBio_short", "2016_X_PacBio_short", "2016_-_PacBio_short"),
  groups = c("Xpedition 2015", "Xpedition 2016", "Lifeguard 2016")
)
taxa::filter_taxa(
  taxmap,
  taxmap$data$read_count$`2015_-_PacBio_short` > 0 |
    taxmap$data$read_count$`2016_X_PacBio_short` > 0 |
    taxmap$data$read_count$`2016_-_PacBio_short` > 0,
  drop_obs = TRUE,
  reassign_obs = FALSE
  ) %>%
metacoder::heat_tree_matrix(
  ...
  node_color_range = metacoder::diverging_palette(),
  edge_size = rowMeans(.$data$read_count[-1]),
  edge_size_range = c(0.0002, 0.03),
  edge_size_trans = "linear",
  node_size_range = c(0.0002, 0.03),
  node_label_size_range = c(0.005, 0.03),
  layout = "davidson-harel",
  initial_layout = "reingold-tilford",
...
```{r}
pseq <- readd(proto_physeq, cache = cache) %>%
  `sample_data<-`(inset2(
    sample_data(.),
    "qual",
    value = fct_collapse(sample_data(.)$qual, "-" = c("", "*"))
...
sample_data(pseq) %>%
  select(.tech = tech, .amplicon = amplicon, .site = site, .year = year, .qual = qual) %>%
  filter(.tech != "Ion Torrent", !is.na(.site), !is.na(.year)) %>%
  unique() %>%
  rowwise() %>%
  pmap_dfr(
    function(.tech, .amplicon, .site, .year, .qual, phyloseq) {
      selection <- with(sample_data(phyloseq), tech == .tech &
          amplicon == .amplicon &
          site == .site &
          year == .year &
          qual == .qual)
      ...
      list(selection = selection))) %>% 
        prune_samples(samples = rowSums(otu_table(.)) > 0) %>%
        prune_taxa(taxa = colSums(otu_table(.)) > 0) %>% {
        full_join(
        distance(., "bray") %>% broom::tidy() %>% rename(bray = distance),
        sample_data(.) %>% rownames_to_column() %$% set_names(x, rowname) %>% dist() %>% broom::tidy()
      )
      } %>%
        mutate(
          tech = .tech,
  ...
  group_by(distance, tech, amplicon, site, year, qual) %>%
  summarize(bray = mean(bray, na.rm = TRUE), n = n()) %>%
  ggplot(aes(distance, bray, size = n)) +
  # geom_hline(data = variog_sills, aes(yintercept = sills), linetype = "dashed", color = "gray50") +
  # geom_vline(data = variog_data, aes(xintercept = range), linetype = "dashed", color = "gray50") +
  # geom_jitter(shape = 46, alpha = 0.5) +
  # geom_line(size = 1.5, alpha = 0.5) +
  ...
  scale_x_continuous(expand = expand_scale(0, 0), limits = c(0, 25), name = "Distance (m)") +
  scale_y_continuous(name = "Community dissimilarity") +
  ggnomics::facet_nested(year + qual + site ~ tech + amplicon)
```
