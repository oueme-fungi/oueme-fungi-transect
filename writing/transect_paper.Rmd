...
bioinf_table <- bind_rows(
  ...
  pivot_longer(asv_table, -1, names_to = "seq_run", values_to = "reads") %>%
    ...
    select(seq_run, reads, step = region, ASVs),
  # pivot_longer(otu, -1, names_to = "seq_run", values_to = "reads") %>%
  #   filter(reads > 0) %>%
  #   mutate_at("seq", chartr, old = "T", new = "U") %>%
  #   inner_join(
  #     select(readd(allseqs, cache = cache), seq = ITS2, long, short, ITS, LSU) %>%
...
names(big_table)[-1] <-
  tibble(seq_run = names(big_table)[-1]) %>%
...
taxon_reads <- 
  ...
  filter(reads > 0) %>%
  left_join(select(datasets, seq_run, amplicon), by = "seq_run") %>%
  expand_grid(
    tibble(
...
(ref:taxa-kingdom) Taxonomic assignment of ASVs to kingdom.
N/A denotes ASVs which could not be identified to kingdom level.
Height of each bar represents the fraction of reads represented by that bar.

```{r taxa-kingdom, fig.cap = "(ref:taxa-kingdom)"}
taxon_reads %>%
  group_by(reference, method, seq_run, kingdom) %>%
  summarize(reads = sum(reads), ASVs = n()) %>%
  ggplot(aes(x = method, y = reads, fill = kingdom)) +
  geom_bar(position = "stack", stat = "identity", color = "white", size = 0.2) +
  facet_wrap(~seq_run + reference, nrow = 1, scales = "free_x") +
  theme(axis.text.x = element_text(angle = 90))
```


(ref:ASV-kingdom) Taxonomic assignment of ASVs to kingdom.
N/A denotes ASVs which could not be identified to kingdom level.
Height of each bar represents the fraction of total ASVs represented by that bar.

```{r ASV-kingdom, fig.cap = "(ref:ASV-kingdom)"}
taxon_reads %>%
  group_by(reference, method, seq_run, kingdom) %>%
  summarize(reads = sum(reads), ASVs = n()) %>%
  ggplot(aes(x = method, y = ASVs, fill = kingdom)) +
  geom_bar(position = "fill", stat = "identity", color = "white", size = 0.2) +
  facet_wrap(~seq_run + reference, nrow = 1, scales = "free_x") +
  theme(axis.text.x = element_text(angle = 90))
```
...
N/A denotes ASVs which were identified as fungi, but could not be identified to phylum level.
Reads which were not identified to kingdom, or which were identified as a non-Fungi kingdom, are not included.
Height of each bar represents the fraction of reads represented by that bar.

```{r taxa-phylum, fig.cap = "(ref:taxa-phylum)"}
taxon_reads %>%
  filter(kingdom == "Fungi") %>%
  group_by(reference, method, phylum, seq_run) %>%
  summarize(reads = sum(reads), ASVs = n()) %>%
  ungroup() %>%
  mutate(
    phylum = ifelse(reads < 0.01 & !is.na(phylum), " other", phylum)
  ) %>%
  group_by(reference, method, phylum, seq_run) %>%
  summarize(reads = sum(reads), ASVs = sum(ASVs)) %>%
  # complete(reference, method, phylum, fill = list(reads = 0, ASVs = 0)) %>%
  ggplot(aes(x = method, y = reads, fill = phylum)) +
  geom_bar(position = "stack", stat = "identity", color = "white", size = 0.2) +
  facet_wrap(~seq_run + reference, nrow = 1, scales = "free_x") +
  theme(axis.text.x = element_text(angle = 90))
...
N/A denotes ASVs which were identified as Ascomycota, but could not be identified to order level.
...
```{r asco-class, fig.cap = "(ref:asco-class)"}
taxon_reads %>%
  filter(phylum == "Ascomycota") %>%
  group_by(reference, method, order, seq_run) %>%
  summarize(reads = sum(reads), ASVs = n()) %>%
  ungroup() %>%
  mutate(
    order = ifelse(reads < 0.01 & !is.na(order), " other", order)
  ) %>%
  group_by(reference, method, order, seq_run) %>%
  summarize(reads = sum(reads), ASVs = sum(ASVs)) %>%
  ggplot(aes(x = method, y = reads, fill = order)) +
  geom_bar(position = "stack", stat = "identity", color = "white", size = 0.2) +
  facet_wrap(~seq_run + reference, nrow = 1, scales = "free_x") +
  theme(axis.text.x = element_text(angle = 90))
...
N/A denotes ASVs which were identified as Basidiomycota, but could not be identified to family level.
Reads which were not identified to phylum, or which were identified as a non-Basidiomycota phylum, are not included.
Height of each bar represents the fraction of total reads represented by that bar.

```{r basid-family, fig.cap = "(ref:basid-family)"}
taxon_reads %>%
  filter(phylum == "Basidiomycota") %>%
  group_by(reference, method, family, seq_run) %>%
  summarize(reads = sum(reads), ASVs = n()) %>%
  ungroup() %>%
  mutate(
    family = ifelse(reads < 0.01 & !is.na(family), " other", family)
  ) %>%
  group_by(reference, method, family, seq_run) %>%
  summarize(reads = sum(reads), ASVs = sum(ASVs)) %>%
  ggplot(aes(x = method, y = reads, fill = family)) +
  geom_bar(position = "stack", stat = "identity", color = "white", size = 0.2) +
  facet_wrap(~seq_run + reference, nrow = 1, scales = "free_x") +
  theme(axis.text.x = element_text(angle = 90))
