...
For each sample, coarse organic debris was removed from the soil surface and a sample of approximately 5cm×5cm×5cm was extracted with a sterilized knife blade and homogenized.
...
The slope of the relationship between PacBio and Illumina richness estimates was not only slightly different from 1, indicating that these two technologies give highly comparable rarefied richness estimates, despite the approximately 200× difference in original sequencing depth.
...
For all sequencing datasets and taxonomic assignment protocols, a higher proportion of reads was assigned than of ASVs, indicating that common ASVs were more likely to be taxonomically identified than rare ASVs (Figure\ \@ref(fig:taxon-chart)).
...
For short amplicon sequencing strategies, the improvement was more modest, because PHYLOTAX could only be applied for ITS2 ASVs with a match to one of the long amplicon ASVs; this was the case for `r readd(taxon_reads_hybrid_phylotax) %>% filter(label %in% readd(phylotax_hybrid)$tree$tip.label, amplicon != "Long") %>% group_by(tech) %>% summarize_at("reads", sum) %$% paste(glue_collapse(percent(reads, format = "f", digits = 0), sep = ", ", last = ", and "), "of reads for the", glue_collapse(tech, sep = ", ", last = ", and "))` short amplicon datasets, respectively.
Deeper long-amplicon seequencing would improve the 
