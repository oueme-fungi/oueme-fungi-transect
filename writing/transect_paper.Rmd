...
write_bib(c("base", "phyloseq", "vegan"), file = "R.bib", tweak = TRUE)
...
Ectomycorrhizas (ECM) form a symbiosis between fungi and plants.
...
Fungal metabarcoding studies using short-read HTS technologies have targeted the ITS1 or ITS2 regions, which provide sufficient resolution to distinguish fungal species in many groups, and which are usually short enough for HTS [@schoch2012;@Lindahl2013].
...
While ITS1 and ITS2 often have suitable variation to distinguish species, they cannot be reliably aligned over the fungal kingdom [@Lindahl2013;@Tedersoo2018].
Additionally, the wide range of length variation of these regions may introduce bias in recovery of different taxa (**ref**).
...
Recent long-read HTS technologies such as Pacific Biosciences Single Molecule Real Time sequencing (PacBio) enable sequencing longer amplicons which include both the ITS regions and the flanking, more conserved SSU and/or LSU regions [@Tedersoo2018].
...
Here we investigated the effects of different sequencing strategies and post-analysis on biological conclusions using measurement of the spatial community turnover rate along a transect in an ECM-dominated Soudanian woodland in Benin.
...
@tedersoo2011 found autocorrelation at ranges <10\ m in Miombo woodland using Sanger sequencing.
...
Sampling was conducted at two sites (Ang: N 9.75456° W 2.14064°; Gan: N 9.75678° W 2.31058 °) approximately 30\ km apart in the _Forêt Classée de l'Ouémé Supérieur_ (Upper Ouémé Forest Reserve) in central Benin.
...
Sequencing results for these samples differed significantly (p < `r rdd(buffer_adonis)["buffer","p.value"] %>% formatC(format = "f")` from permutational MANOVA using 9999 permutations, $R^2 = `r rdd(buffer_adonis)["buffer","R2"] %>% formatC(format = "f", digits = 2)`$) from samples preserved using the Xpedition^TM^ lysis buffer (Supplementary Figures \@ref(fig:buffer-pcoa), \@ref(fig:buffer-compare-long), and \@ref(fig:buffer-compare-short)); as such these samples were excluded from our spatial analyses.
...
The long amplicon (approximately 1500\ bp) targeted the full ITS region including the 5.8S rDNA and approximately 950\ bp at the 5' end of the LSU, including the first three variable regions (Figure\ \@ref(fig:rDNA)), using ITS1 [@white1990amplification] as the forward primer and LR5 [@vilgalys1990] as the reverse primer.
...
Sequences were then dereplicated and denoised into amplicon sequencing variants (ASVs) using DADA2 version `r packageVersion("dada2")` [R package `dada2`, @callahan2016; @callahan2019].
...
Taxonomic annotations of the Ribosomal Data Project's LSU fungal training set (RDP) version 11.5 [@cole2014] and Warcup ITS training set [@deshpande2016] were mapped to the taxonomic classification system used in the Unite database version 8 [@nilsson2019a].
...
The fungal communities recovered by the three sequencing strategies that were successfully demultiplexed (Illumina, PacBio Short, PacBio Long) were compared by permutational MANOVA.
In order to detect bias at larger taxonomic scales, ASVs were clustered according to the assigned taxonomic class.
Only samples where all three strategies yielded at least 100\ fungal reads (`r phyloseq::nsamples(rdd(tech_class_table))/3` samples), and classes which represented at least 1% of reads in at least one sample (`r phyloseq::ntaxa(rdd(tech_class_table))` classes), were included.
Permutational MANOVA included three terms: an indicator for soil sample, comprising all spatiotemporal effects; amplicon length (long vs. short); and sequencing technology (Illumina MiSeq vs. PacBio RS II).
The marginal significance of each term for explaining variation in the Bray-Curtis community dissimilarity matrix was performed using the `adonis2` function in the R package `vegan` [@R-vegan], with 9999 permutations.
...
Demultiplexing, primer trimming, and quality filtering reduced these totals by `r (1-rdd(filtercounts_full)/rdd(rawcounts))["pb_500"] %>% percent(format = "d")` for PacBio long reads, but only by `r (1-rdd(filtercounts_full)/rdd(rawcounts))["pb_483"] %>% percent(format = "d")` for PacBio short reads, resuling in a similar numbe rof filtered reads for the two strategies.
Losses in demultiplexing, trimming, and quality filtering were intermediate for Ion Torrent and Illumina, with `r (1-rdd(filtercounts_full)/rdd(rawcounts))[c("is_057", "SH-2257")] %>% percent(format = "d") %>% text_list()` loss, respectively.
In contrast, extraction of only the ITS2 region before quality filtering resulted in the loss of `r percent(1-rdd(filtercounts_ITS2)['pb_500']/rdd(rawcounts)['pb_500'], format = "d")` of trimmed long amplicon PacBio reads, `r percent(1-rdd(filtercounts_ITS2)['pb_483']/rdd(rawcounts)['pb_483'], format = "d")` of trimmed short amplicon PacBio reads, and `r percent(1-rdd(filtercounts_ITS2)['is_057']/rdd(rawcounts)['is_057'], format = "d")` of trimmed Ion Torrent reads.
...
Although the length profile of the three sequencing runs were similar, Illumina MiSeq had the largest fraction of reads near the top of the range, followed by Ion Torrent Ion S5 and PacBio RS II (Supplementary Figure\ \@ref(fig:full-length)b).
The difference in length distributions was statistically significant due to the large sample size (Kruskal-Wallis statistic = `r rdd(length_kw)$statistic`, $p < 2.2\times10^{-16}$), but the difference between means was fairly small, with mean amplicon lengths of `r filter(rdd(reads_table), region == "short") %>% group_by(seq_run) %>% summarize(length = weighted.mean(length, reads)) %>% deframe() %>% magrittr::extract(c("pb_483", "is_057", "SH-2257")) %>% text_list(digits = 0)` bp for PacBio, Ion Torrent, and Illumina, respectively.
In contrast, the length of the long amplicon reads varied widely, from `r filter(rdd(reads_table), region == "long") %$% range(length) %>% paste(collapse = " to ")` bp, with a mean of `r filter(rdd(reads_table), region == "long") %$% weighted.mean(length, reads) %>% round()` bp.

The length distribution of the different regions extracted from the long amplicon are shown in Supplementary Figure\ \@ref(fig:region-lengths).
ITS1 showed the greatest length variability (mean $\pm$ standard deviation: `r rdd(reads_table) %>% filter(region == "ITS1", seq_run == "pb_500") %$% reldist::wtd.mean(length, weight = reads) %>% round()` $\pm$ `r rdd(reads_table) %>% filter(region == "ITS1", seq_run == "pb_500") %$% reldist::wtd.var(length, weight = reads) %>% sqrt() %>% round()`\ bp), followed by ITS2 (`r rdd(reads_table) %>% filter(region == "ITS2", seq_run == "pb_500") %$% reldist::wtd.mean(length, weight = reads) %>% round()` $\pm$ `r rdd(reads_table) %>% filter(region == "ITS2", seq_run == "pb_500") %$% reldist::wtd.var(length, weight = reads) %>% sqrt() %>% round()`\ bp) and the variable regions in LSU (D2: `r rdd(reads_table) %>% filter(region == "D2", seq_run == "pb_500") %$% reldist::wtd.mean(length, weight = reads) %>% round()` $\pm$ `r rdd(reads_table) %>% filter(region == "D2", seq_run == "pb_500") %$% reldist::wtd.var(length, weight = reads) %>% sqrt() %>% round()`\ bp; D3: `r rdd(reads_table) %>% filter(region == "D3", seq_run == "pb_500") %$% reldist::wtd.mean(length, weight = reads) %>% round()` $\pm$ `r rdd(reads_table) %>% filter(region == "D3", seq_run == "pb_500") %$% reldist::wtd.var(length, weight = reads) %>% sqrt() %>% round()`\ bp; D1: `r rdd(reads_table) %>% filter(region == "D1", seq_run == "pb_500") %$% reldist::wtd.mean(length, weight = reads) %>% round()` $\pm$ `r rdd(reads_table) %>% filter(region == "D1", seq_run == "pb_500") %$% reldist::wtd.var(length, weight = reads) %>% sqrt() %>% round()`\ bp).
Approximately `(rdd(reads_table) %>% filter(region == "LSU4", seq_run == "pb_500", length > 200) %$% sum(reads)/rdd(reads_table) %>% filter(region == "LSU4", seq_run == "pb_500") %$% sum(reads)) %>% signif(1) %>% percent()` of reads included an intron of 40--60 bp in the LSU4 region, not visible in Supplementary Figure\ \@ref(fig:region-lengths) due to rarity.
...
The abundance of the second ASV was `r rdd(agaricus_reads) %>% as.data.frame() %>% {.[["f2ab49e8"]]/.[["39d80c19"]]} %>% keep(~. > 0) %>% percent(format = "f", digits = 1) %>% paste(collapse = " and ")` that of the primary *A. bisporus* ASV in the two Illumina positive controls.
...
ASVs shared between all datasets represented `r paste(range(as.numeric(unlist(rdd(venn_ASV)["1111", c("frac_pb_500", "frac_pb_483", "frac_SH-2257", "frac_is_057")]))*100), collapse = "--")`% of the reads for the long and short PacBio datasets, Illumina dataset, and Ion Torrent dataset, respectively.
These fractions increased to `r paste(range(as.numeric(unlist(rdd(venn_OTU)["1111", c("frac_pb_500", "frac_pb_483", "frac_SH-2257", "frac_is_057")]))*100), collapse = "--")`% when differences at the intra-species scale were removed by clustering the ASVs into 97% OTUs.
...
According to the PHYLOTAX assignments, Fungi represented `r percent(fungi_asv["PacBio_Long"], format = "f", digits = 0)` of the ASVs and `r percent(fungi_read["PacBio_Long"], format = "f", digits = 0)` of the reads in the long amplicon library, compared to `r paste(percent(range(fungi_asv[c("PacBio_Short", "Illumina_Short", "Ion.Torrent_Short")]), format = "f", digits = 1), collapse = "–")` of the ASVs and `r paste(percent(range(fungi_read[c("PacBio_Short", "Illumina_Short", "Ion.Torrent_Short")]), format = "f", digits = 1), collapse = "–")` of the reads in the short amplicon library.
Fungi categorized as ECM made up `r percent(ecm_asv["pb_500"], format = "f", digits = 1)` of ASVs and `r percent(ecm_read["pb_500"], format = "f", digits = 1)` of reads in the long amplicon library, and `r paste(percent(range(ecm_asv[c("pb_483", "SH-2257", "is_057")]), format = "f", digits = 1), collapse = "–")` of the ASVs and `r paste(percent(range(ecm_read[c("pb_483", "SH-2257", "is_057")]), format = "f", digits = 1), collapse = "–")` of the reads in the short amplicon library.
...
Figure\ \@ref(fig:ppcoa)

(ref:ppcoa-short) Partial Principal Coordinates Analysis (PPCoA) of fungal community using different metabarcoding approaches
...
Abbreviations represent the approximate direction of increase for the classes and families with the highest scores: `r glue::glue_data(rdd(tech_class_pcoa_scores), "**{abbrev}**: {class}") %>% glue::glue_collapse(sep = ", ", last = ", and ")`; `r glue::glue_data(rdd(tech_ecm_fam_pcoa_scores), "**{abbrev}**: {family}") %>% glue::glue_collapse(sep = ", ", last = ", and ")`.

```{r ppcoa, fig.cap = "(ref:ppcoa-short), (ref:ppcoa)", fig.scap = "(ref:ppcoa-short)"}
ggpubr::ggarrange(
  rdd(tech_class_pcoa_plot),
  rdd(tech_ecm_fam_pcoa_plot),
  common.legend = TRUE,
  legend = "bottom",
  labels = "auto"
)
...
Although a greater fraction of PacBio raw reads were ultimately mapped to ASVs (`r percent(rdd(bioinf_table)["ITS2","PacBio_Short_reads"]/rdd(rawcounts)["pb_483"], format = "f", digits = 0)`) compared to Illumina (`r percent(rdd(bioinf_table)["ITS2","Illumina_Short_reads"]/rdd(rawcounts)["SH-2257"], format = "f", digits = 0)`) or Ion Torrent (`r percent(rdd(bioinf_table)["ITS2","Ion Torrent_Short_reads"]/rdd(rawcounts)["is_057"], format = "f", digits = 0)`),
...
Contrary to expectations, Illumina showed a slightly higher fraction of longer ITS2 sequences than Ion Torrent, which in turn showed slightly longer sequences than PacBio (Supplementary Figures\ \@ref(fig:full-length) and \@ref(fig:ITS2-length)).

Of long amplicon reads, `r percent(rdd(venn_ASV)["1000", "frac_pb_500"])` belonged to ASVs which occurred only in the long amplicon dataset, and clustering at 97% similarity only reduced this fraction to `r percent(rdd(venn_OTU)["1000", "frac_pb_500"])`.
Additionally, ITS2 sequences extracted from the long amplicon dataset included some sequences that were much shorter than those recovered from the short amplicon datasets \@ref(fig:ITS2-length).
Taxonomic assignments revealed that many of these non-shared sequences fall outside kingdom Fungi (Supplementary Figure\ \@ref(fig:heattree-length-compare)), and that in particular the short ITS2 sequences are mostly Alveolates (Supplementary Figure\ \@ref(fig:short-its)).
Within Fungi, the short amplicon datasets recovered more Sordariomycetes (Figure \@ref(fig:pcoa)), possibly because this group was represented by diverse but rare taxa which remained undetected in the PacBio datasets due to the lower sequencing depth.
...
ASV recovery for long amplicons using DADA2 was dramatically improved (`r percent(rdd(region_table)$reads[rdd(region_table)$region == "long"]/as.integer(gsub(" ", "", rdd(demuxcounts)["pb_500"])), format = "d")` to `r percent((rdd(asvcounts) %>% filter(seq_run == "pb_500", step == "long") %>% pull(reads))/as.integer(gsub(" ", "", rdd(demuxcounts)["pb_500"])), format = "d")` of reads) by denoising homologous subregions independently using the `lsux` and `tzara` packages.
...
For instance, `r filter(rdd(tax_chart), tech == "Illumina", Algorithm == "Consensus", rank %in% c("genus", "family"), type == "Reads", reference == "Unite") %>% pull(frac) %>% percent(format = "f", digits = 0) %>% rev() %>% text_list()` of Illumina reads were assigned to genus and family, respectively, by the strict consensus of methods, but PHYLOTAX increased this fraction to `r filter(rdd(tax_chart), tech == "Illumina", Algorithm == "PHYLOTAX", rank %in% c("genus", "family"), type == "Reads", reference == "Unite") %>% pull(frac) %>% percent(format = "f", digits = 0) %>% rev() %>% text_list()`.
...
There was little variation in the results between the Illumina and long-amplicon PacBio datasets for both the total fungal community and the ECM community, with best fit estimates ranging from `r rdd(variofit_table) %>% filter((amplicon == "Long") | (tech == "Illumina"), metric == "Bray-Curtis", term == "range") %$% range(estimate) %>% format(digits = 0) %>% paste(collapse = "--")`\ m.
The 95% confidence interval was substantially wider than this variation, generally covering a range of `r rdd(variofit_table) %>% filter((amplicon == "Long") | (tech == "Illumina"), metric == "Bray-Curtis", term == "range") %$% range(c(conf.low, conf.high), na.rm = TRUE) %>% format(digits = 0) %>% paste(collapse = "--")`\ m.
All of these values are smaller than the 65\ m reported by @Bahram2013, also from an ECM woodland habitat in Benin.
The PacBio short amplicon dataset shows a longer spatial range, of `r rdd(variofit_table) %>% filter(amplicon == "Short", tech == "PacBio", metric == "Bray-Curtis", term == "range", guild == "All Fungi") %$% range(estimate) %>% unique() %>% format(digits = 0) %>% paste(collapse = "--")`\ m for the total fungal community and `r rdd(variofit_table) %>% filter(amplicon == "Short", tech == "PacBio", metric == "Bray-Curtis", term == "range", guild == "ECM") %$% range(estimate) %>% unique() %>% format(digits = 0) %>% paste(collapse = "--")`\ m for the ECM community, in both cases with wide confidence intervals spanning `r rdd(variofit_table) %>% filter(amplicon == "Short", tech == "PacBio", metric == "Bray-Curtis", term == "range") %$% range(c(conf.high, conf.low), na.rm = TRUE) %>% unique() %>% format(digits = 0) %>% paste(collapse = "--")`\ m.
...
The spatiotemporal distance-decay fit estimated the temporal turnover range as `r rdd(variofit_table) %>% filter(amplicon == "Long", tech == "PacBio", metric == "Bray-Curtis", term == "timerange", guild == "All Fungi") %$% range(estimate, na.rm = TRUE) %>% unique() %>% formatC(format = "f", digits = 1) %>% paste(collapse = "--")`\ years for the total fungal community and `r rdd(variofit_table) %>% filter(amplicon == "Long", tech == "PacBio", metric == "Bray-Curtis", term == "timerange", guild == "ECM") %$% range(estimate, na.rm = TRUE) %>% unique() %>% formatC(format = "f", digits = 1) %>% paste(collapse = "--")`\ years for the ECM community, but with overlapping confidence intervals.
...
- Raw data?
- ASV sequences to Unite
- alignment and trees to TreeBase
- R packages on CRAN or Dryad (plus Github)
...
This project was funded by Formas grant XXX

Ylva for PCR and sequence prep.
...
(ref:heattree-length-compare) Red nodes (edges) represent increased read (ASV) count for the short amplicon library, while blue-green nodes (edges) represent increased read (ASV) count for the long amplicon library.
...
(ref:buffer-pcoa-s) Unconstrained Principal Coordinate Analysis ordination of samples preserved with different buffers

(ref:buffer-pcoa) For each sequencing strategy, only those sampling locations with more than 100 reads each for all three year/preservation buffer combinations were included.

```{r buffer-pcoa, fig.cap="(ref:buffer-pcoa-s). (ref:buffer-pcoa)"}
plot(rdd(buffer_asv_pcoa), type = "none")
points(
  rdd(buffer_asv_pcoa),
  select = with(
    as_tibble(phyloseq::sample_data(rdd(buffer_asv_table))),
...
points(
  rdd(buffer_asv_pcoa),
  select = with(as_tibble(phyloseq::sample_data(rdd(buffer_asv_table))),
...
# legend(
#   "topright",
#   legend = with(as_tibble(phyloseq::sample_data(rdd(buffer_asv_table))), paste(tech, amplicon)) %>%
#     unique() %>%
#     sort(),
#   lty = 1:3
# )
# vegan::ordiellipse(
#   rdd(buffer_asv_pcoa),
#   groups = with(as_tibble(phyloseq::sample_data(rdd(buffer_asv_table))), paste(tech, amplicon)),
#   lty = 1:3
# )
```
