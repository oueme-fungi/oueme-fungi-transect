...
The short amplicon (approximately 300 bp) included the full ITS2 region as well as parts of the flanking 5.8S and large subunit (LSU) rDNA, using gITS7 [@ihrmark2012] as the forward primer and a mix of ITS4 [@white1990amplification] and ITS4a [@urbina2016] (hereafter ITS4m) as the reverse primer.
The long amplicon (approximately 1550 bp) included approximately 12 bp at the 3' end of the ribosomal small subunit (SSU) RNA, the full ITS region including the 5.8S rDNA, and approximately 950 bp at the 5' end of the LSU, including the first three variable domains (figure \@ref(fig:rDNA)), using ITS1 [@white1990amplification] as the forward primer and LR5 [@vilgalys1990] as the reverse primer.
...
(ref:rDNA) Portion of the rDNA showing the 5.8S rDNA, partial SSU and LSU rDNA (thick lines), and internally transcribed spacer (ITS) regions (thin black lines), as well as primer sites used in this study (red and blue arrows) and the first three variable regions in LSU (grey).)
...
We attempted to denoise both long and short amplicons using `DADA2` version `r packageVersion("dada2")` [@callahan2016; @callahan2019] according to the steps outlined in  the supplementary information in @callahan2019, but only a very small fraction of the long amplicon reads were mapped to amplicon sequence variants (ASVs) at the end of this process.
We believe that this was due to `DADA2`'s requirement that the seed sequence of each ASV not be a singleton read.
For the combination of long reads, relatively high diversity, and relatively low sequencing depth in this study, this requirement was not met except by the most abundant sequences.
...
First, the 5.8S rRNA was located in each read by searching for Rfam model RF0002 [@kalvari2018] using `cmsearch` from Infernal 1.1.2 [@nawrocki2013], and all bases before the 5.8S were assigned to ITS1.
...
The fragment of each read beginning with the 5.8S rRNA was then aligned to a CM based on a modification of the fungal 28S RNA seed alignment from the Ribosomal Data Project (RDP) release 11.5 [@glockner2017; @cole2014] using `cmalign` from Infernal 1.1.2 [@nawrocki2013].
The reference line in the CM alignment for each read were then used to split the reads into alternating more-conserved and less-conserved regions:
ITS1, 5.8S, ITS2, LSU1, D1, LSU2, D2, LSU3, D3, LSU4,
where LSU1-4 represent the conserved regions of the 28S rRNA flanking the variable D1-3 regions.
For short amplicons, only 5.8S, ITS2, and LSU1 were extracted.
The process of extracting the regions is automated in the new R package `LSUx`, available on github (https://github.com/brendanf/LSUx).
...
Because ITS2 is one of the most variable regions in the amplified regions, reads with identical ITS2 regions are expected have highly similar sequences in the other regions, unless the amplicon was chimeric.
Thus, the concatenated ASVs representing each long read were aligned in R using the `DECIPHER` package [@wright2015], and outlier sequences (interpreted as likely chimeras) were removed from each alignment using the `odseq` package [@jehl2015].
...
For comparison with clustering-based OTU methods, ASVs were clustered at 97% similarity using VSEARCH v2.9.1 [@rognes2016].
...
Taxonomic assignment of ASVs which were represented in the long amplicon dataset was refined using the phylogenetic tree.
*Describe this more.*
