...
Approximately 8% of described fungal species are believed to take part in ECM symbiosis [@ainsworth2008ainsworth;@Rinaldi2008].
Although only about 2% of land plant species form ECM, these include ecologically and economically important stand-forming trees belonging to both temperate/boreal groups such as Pinaceae and Fagaceae, and tropical groups such as Dipterocarpaceae, and Fabaceae tr. Amherstieae [@brundrett2017a].
...
The advent of high-thoughput sequencing (HTS) has facilitated such studies by providing enough sequencing depth for metabarcoding of bulk environmental samples such as soils [@Lindahl2013].
...
This has the potential to improve taxonomic placement of sequences that lack close database matches, and to allow the alignment of metabarcoding reads.
...
Turnover scale is often measured by the range at which a Mantel correlogram indicates significant autocorrelation, or by fitting a function to an empirical distance-decay curve community dissimilarity vs. distance [@legendre2012].
...
Forward and reverse Illumina reads were truncated after the first base with quality score <= 10, and then reads with more than 3 expected errors in either read were discarded.
Forward and reverse reads were denoised using `DADA2` version `r packageVersion("dada2")` [@callahan2016] using seperate error models and pseudo-pooling, and then forward and reverse reads were merged.
...
The count and sample distribution of reads assigned to each ASV were calculated in order to form a sample × ASV community matrix.
...
For datasets where the Mantel corellogram indicated spatial correlation between samples taken in seperate years, the model was re-fit with an additional term to represent temporal correlation:
...
95% confidence intervals were calculated for the spatial and temporal range parameters by profiling using the `MASS` package in R.

# Results

The number of sequencing reads and ASVs at each stage in the bioinformatics pipeline are shown in Table\ \@ref(tab:bioinfo).
Long (short) amplicon sequencing with PacBio yielded `r readd(rawcounts)["pb_500"]` (`r readd(rawcounts)["pb_483"]`) circular consensus reads, of which `r readd(demuxcounts)["pb_500"]` (`r readd(demuxcounts)["pb_483"]`) were successfully trimmed and demultiplexed.
Illumina sequencing yielded `r readd(rawcounts)["SH-2257"]` read pairs, of which `r readd(demuxcounts)["SH-2257"]` were successfully trimmed and demultiplexed, and `r readd(filtercounts_full)["SH-2257"]` passed quality filtering.
Ion Torrent sequencing yielded `r readd(demuxcounts)["is_057"]` demultiplexed reads.
`r readd(filtercounts_full)['pb_500']` (`r percent(readd(filtercounts_full)['pb_500']/readd(demuxcounts)['pb_500'], digits = 1, format = "f")`) of long amplicon PacBio reads passed length and quality filtering stages, compared with `r readd(filtercounts_full)['pb_483']` (`r percent(readd(filtercounts_full)['pb_483']/readd(demuxcounts)['pb_483'], digits = 1, format = "f")`) of short amplicon PacBio reads, `r readd(filtercounts_full)['SH-2257']` (`r percent(readd(filtercounts_full)['SH-2257']/readd(demuxcounts)['SH-2257'], digits = 1, format = "f")`) of Illumina read pairs, and `r readd(filtercounts_full)['is_057']` (`r percent(readd(filtercounts_full)['is_057']/readd(demuxcounts)['is_057'], digits = 1, format = "f")`) of Ion Torrent reads.
...
The 5.8S rDNA was located in `r readd(regioncounts)['pb_500']` long amplicon PacBio reads (`r percent(readd(regioncounts)['pb_500']/readd(demuxcounts)['pb_500'], digits = 1, format = "f")` of demultiplexed total), `r readd(regioncounts)['pb_483']` short amplicon PacBio reads (`r percent(readd(regioncounts)['pb_483']/readd(demuxcounts)['pb_483'], digits = 1, format = "f")`), and `r readd(regioncounts)['is_057']` Ion Torrent reads (`r percent(readd(regioncounts)['is_057']/readd(demuxcounts)['is_057'], digits = 1, format = "f")`).
Length and quality filtering resulted in `r readd(filtercounts_ITS2)['pb_500']` (`r percent(readd(filtercounts_ITS2)['pb_500']/readd(demuxcounts)['pb_500'], digits = 1, format = "f")` of demultiplexed total) ITS2 sequences from the long amplicon PacBio reads, `r readd(filtercounts_ITS2)['pb_483']` (`r percent(readd(filtercounts_ITS2)['pb_483']/readd(demuxcounts)['pb_483'], digits = 1, format = "f")`) ITS2 sequences from the short amplicon PacBio reads, and `r readd(filtercounts_ITS2)['is_057']` (`r percent(readd(filtercounts_ITS2)['is_057']/readd(demuxcounts)['is_057'], digits = 1, format = "f")`) ITS2 sequences from the Ion torrent reads.
...
```{r venn, fig.cap="(ref:venn)", fig.show="hold", out.width = "90%"}
...
grid::grid.legend(
  ...
  vp = grid::viewport(y = unit(0.1, "npc"))
)
...
#   vp = grid::viewport(y = unit(0.2, "npc"))
# )
```
...
In contrast, the length of the long amplicon reads varied widely, from `r filter(readd(reads_table), region == "long") %$% range(length) %>% paste(collapse = " to ")` bp, with a median of `r filter(readd(reads_table), region == "long") %$% reldist::wtd.quantile(length, weight = reads)` bp.
...
Although a greater fraction of PacBio raw reads were ultimately mapped to ASVs (`r percent(readd(bioinf_table)["ITS2","PacBio_Short_reads"]/readd(rawcounts)["pb_483"], format = "f", digits = 1)`) compared to Illumina (`r percent(readd(bioinf_table)["ITS2","Illumina_Short_reads"]/readd(rawcounts)["SH-2257"], format = "f", digits = 1)`) or Ion Torrent (`r percent(readd(bioinf_table)["ITS2","Ion Torrent_Short_reads"]/readd(rawcounts)["is_057"], format = "f", digits = 1)`),
...
`r percent(readd(venn_ASV)["1000", "frac_pb_500"])` of long amplicon reads beloged to ASVs which were not found in any short amplicon dataset, and clustering at 97% similarity only reduced this fraction to `r percent(readd(venn_OTU)["1000", "frac_pb_500"])`.
...
Additionally, several groups showed anomalous detection in either the long or short datasets, such as *Tulasnella*, *Tremellodendron*, and *Marcelleina* in the long amplicon dataset, and *Xylaria* and *Myerozyma* in the short amplicon datasets.
These differences may be due to primer mismatches in these groups.
It is of note that the three aforementioned groups which were more common in the long amplicon dataset are ECM.
...
ASV recovery for long amplicons was dramatically improved (`r percent(readd(region_table)$reads[readd(region_table)$region == "long"]/as.integer(gsub(" ", "", readd(demuxcounts)["pb_500"])), format = "d")` to `r percent((readd(asvcounts) %>% filter(seq_run == "pb_500", step == "long") %>% pull(reads))/as.integer(gsub(" ", "", readd(demuxcounts)["pb_500"])), format = "d")`) by denoising homologous subregions independently.
...
It may also be adaptable to Oxford Nanopore sequencing, which has hitherto not been successfully applied to complex community metabarcoding [@loit2019].
...
For instance, `r filter(readd(tax_chart), tech == "Illumina", Algorithm == "Consensus", rank %in% c("genus", "family"), type == "Reads", reference == "Unite") %>% pull(frac) %>% percent(format = "d") %>% rev() %>% text_list()` of Illumina reads were assigned to genus and family, respectively, by the strict consensus of methods, but PHYLOTAX increased this fraction to `r filter(readd(tax_chart), tech == "Illumina", Algorithm == "PHYLOTAX", rank %in% c("genus", "family"), type == "Reads", reference == "Unite") %>% pull(frac) %>% percent(format = "d") %>% rev() %>% text_list()`.
...
(ref:full-length) Comparison of the length of denoised sequences from different length amplicons and sequencing technologies.
Length distribution (*a*, *c*) and empirical cumulative distribution function (ECDF; *b*, *d*) for short (*a*, *b*) and long (*c*, *d*) amplicons, respectively.
...
(ref:region-lengths) Length distribution for each region extracted from long amplicons.  Filtering limits are shown as dashed vertical lines.

```{r region-lengths, fig.cap = "(ref:region-lengths)", fig.height = 6}
