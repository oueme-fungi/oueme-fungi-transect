...
Samples preserved using LifeGuard were first centrifuged at XX ×g for Y minutes, after which the supernatant was removed and DNA was extracted from the remaining soil using the Soil/Fecal Prep kit as for the other samples.
...
Additionally, a hybrid method, "PHYLOTAX+C", was applied to the short amplicon datasets, in which assignments from PHYLOTAX were used for ASVs which could be linked by an identical ITS2 region to a long amplicon, and assignments from Consensus were used for the remaining ASVs.
...
readd(allseqs, cache = cache) %>% filter(hash %in% taxa_names(agaricus_reads)) %$%
  set_names(short, hash) %>%
  discard(is.na) %>%
  Biostrings::RNAStringSet() %>%
  Biostrings::writeXStringSet(here("temp/agaricus.fasta"))
...
venn_ASV %>%
  ...
  mutate_all(~ifelse(
    . == "",
    kableExtra::cell_spec("      ", background = empty_background, background_as_tile = FALSE), .)) %>%
...
linesep <- venn_OTU %>%
  ...
  head(length(.) - 2) %>%
  c("\\midrule")
venn_OTU %>% as.matrix() %>% equals("") %>% which()
venn_OTU %>%
  filter(!is.na(OTUs)) %>%
  mutate_all(~ifelse(. == "", kableExtra::cell_spec("      ", background = empty_background, background_as_tile = FALSE), .)) %>%
  set_names(gsub("_[[:alpha:]]+[-._]\\d+$", "", names(.))) %>%
  set_names(gsub("_", " ", names(.))) %>%
  kable(
    booktabs = TRUE,
...
eulerr::venn(deframe(select(vennplot_OTU, set, OTUs)) %>% replace_na(0)) %>%
  plot(
    legend = FALSE,
    labels = FALSE,
...
(ref:tax-chart) Fraction of ASVs (left) and reads (right) assigned to each taxonomic rank, for different sequencing technologies (PacBio RS II, Ion Torrent Ion S5), amplicons (Long, Short), reference databases (Unite, Warcup, RDP), and assignment algorithms (PHYLOTAX, PHYLOTAX+C, Consensus, RDPC, SINTAX, IDTAXA).
...
readd(taxon_reads, cache = cache) %>%
  ...
  mutate(
    reference = map2(reference, amplicon, ~ if (.x == "All") c("Unite", "Warcup", if (.y == "Long") "RDP" else NULL) else .)
  ) %>%
  unnest(reference) %>%
...
light_plan %>%
  ...
  xlab("Distance (m)") +
  ggnomics::facet_nested(metric + tech + amplicon + method ~ guild, scales = "fixed", nest_line = TRUE) +
...
variog_data <- light_plan %>%
  ...
  select(variofitST, variogST, guild, metric, tech, amplicon, algorithm) %>%
  mutate_at(c("guild", "metric", "tech", "amplicon", "algorithm"), str_replace_all, "\"", "") %>%
  mutate(
    guild = plyr::mapvalues(guild, c("fungi", "ecm"), c("All Fungi", "ECM")),
    metric = plyr::mapvalues(metric, c("bray", "wunifrac"), c("Bray-Curtis", "W-UNIFRAC")),
    amplicon = factor(
...
variog_empir %>%
  ...
  ggnomics::facet_nested(metric + tech + amplicon + algorithm ~ guild, scales = "free", nest_line = TRUE) +
  theme(strip.background = element_blank())
...
variog_data %>%
  ...
  select(Amplicon = amplicon, Tech = tech, Guild = guild, Algorithm = algorithm,
         Metric = metric, Range = range, Anisotropy = anis) %>%
  ...
  select(Amplicon, Tech, Guild, Algorithm, Metric, "Space Range" = Range, "Time Range") %>%
...
full_dens <- readd(reads_table, cache = cache) %>%
...
its2 <- readd(reads_table, cache = cache) %>%
...
reads_plot <- readd(qstats_n, cache = cache) %>%
...
conc_plot <- readd(proto_physeq, cache = cache) %>% 
        sample_data(.) %>%
