...
# plot(rdd(tech_class_pcoa), type = "none")
# points(
#   rdd(tech_class_pcoa),
#   select = with(
#     rdd(tech_class_sample_data),
#     tech == "PacBio" & amplicon == "Short"
#   ),
#   col = "green",
#   pch = 1
# )
...
#   select = with(
#     rdd(tech_class_sample_data),
#     tech == "Illumina" & amplicon == "Short"
#   ),
#   col = "red",
#   pch = 3
# )
# with(
#   rdd(tech_class_pcoa_scores),
#   text(x = MDS1*2, y = MDS2*2, labels = abbrev)
# )
# legend(
#   "bottomleft",
#   legend = c("Illumina Short", "PacBio Short", "PacBio Long"),
#   col = c("red", "green", "blue"),
#   pch = c(3, 1, 2)
# ) 
# 
# plot(rdd(tech_ecm_fam_pcoa), type = "none")
# points(
#   rdd(tech_ecm_fam_pcoa),
#   select = with(
#     rdd(tech_ecm_fam_sample_data),
#     tech == "PacBio" & amplicon == "Short"
#   ),
#   col = "green",
#   pch = 1
...
# )
# with(
#   rdd(tech_ecm_fam_pcoa_scores),
#   text(x = MDS1*3, y = MDS2*3, labels = abbrev)
# )
# legend(
#   "bottomleft",
#   legend = c("Illumina Short", "PacBio Short", "PacBio Long"),
...
```{r}
tech_class_nmds <- vegan::metaMDS(
  phyloseq::otu_table(rdd(tech_class_table))
)

tech_class_nmds_scores <-
  vegan::scores(tech_class_nmds, display = "species") %>%
  as_tibble(rownames = "class") %>%
  mutate(L = sqrt(NMDS1^2 + NMDS2^2)) %>%
  filter(!is.nan(L)) %>%
  filter(L >= max(L) / 2) %>%
  mutate(abbrev = substr(class, start = 1, stop = 2))

plot(tech_class_nmds, type = "n")
vegan::ordispider(
  tech_class_nmds,
  groups = with(
    rdd(tech_class_sample_data),
    paste(buffer, year, site, x)
    ),
  col = "gray80"
  )
text(
  x = tech_class_nmds_scores$NMDS1,
  y = tech_class_nmds_scores$NMDS2,
  labels = tech_class_nmds_scores$abbrev
  )
points(tech_class_nmds, select = with(rdd(tech_class_sample_data),
                                     tech == "PacBio" & amplicon == "Short" ),
       col = "green", pch = 1)
points(tech_class_nmds, select = with(rdd(tech_class_sample_data),
                                     tech == "PacBio" & amplicon == "Long"),
...
points(tech_class_nmds, select = with(rdd(tech_class_sample_data),
                                     tech == "Illumina" & amplicon == "Short"),
       col = "red", pch = 3)
legend("topright", legend = c("Illumina Short", "PacBio Short", "PacBio Long"), col = c("red", "green", "blue"), pch = c(3, 1, 2))
```
...
Results are qualitatively similar between the two amplicon libraries and between PacBio and Illumina sequencing, with significant autocorrelation ($p < 0.05$, filled red circles) in the Bray-Curtis metric at ranges of up to 2--3\ m for the total fungal community, and 1--2\ m for the ECM community.
...
There was little variation in the results between the Illumina and long-amplicon PacBio datasets for both the total fungal community and the ECM community, with best fit estimates ranging from 13--19\ m.
The 95% confidence interval was substantially wider than this variation, generally covering a range of 6--35\ m.
All of these values are smaller than the 65\ m reported by @Bahram2013, also from an ECM woodland habitat in Benin.
The PacBio short amplicon dataset shows a longer spatial range, of 30\ m for the total fungal community and 42--43\ m for the ECM community, in both cases with wide confidence intervals spanning approximately 15--60\ m.
...
The spatiotemporal distance-decay fit estimated the temporal turnover range as 3.1\ years for the total fungal comunity and 4\ years for the ECM community, but with overlapping confidence intervals.
