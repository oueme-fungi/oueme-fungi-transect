...
However, only `r region_table$ASVs[region_table$region=="long"]` amplicon sequence variants (ASVs), were obtained.
We believe that this was due to `DADA2`'s requirement that the seed sequence of each ASV be represented by at least two error-free reads.
...
For the combination of long reads (median *X* bp after trimming) and moderate error rate (*X*% mean ccs quality score) in this study, the expected number of reads to achieve two error-free reads is `r round(2/(0.995)^1550)`.
In comparison, the equivalent requirement for the short amplicon (median *X* bp after trimming, mean ccs quality score *Y*%) is only `r round(2/(0.999)^350)`
Given the high diversity relative to sequencing depth in this study (*X* ASVs from PacBio short amplicons, *Y* total long amplicon reads), this requirement was not met for the long amplicons except by the most abundant sequences.
...
Long (short) amplicon sequencing with PacBio yielded *X* (*Y*) circular consensus reads, of which *A* (*B*) were successfully demultiplexed.
The 5.8S region was detected in *C* (*D*) reads.
Ion Torrent sequencing yielded *E* demultiplexed reads, with the 5.8S region detected in *F*.
...
extree <-
  tidytree::as.treedata(ape::read.tree(text = "(A:1,((B:1,C:1):1,((E:1,F:1):1,D:1):1):1);")) %>%
  tidytree::full_join(
