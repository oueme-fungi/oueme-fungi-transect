...
fungi_read <- readd(taxdata, cache = cache) %>%
  taxa::filter_taxa(
    taxon_names == "Fungi",
    reassign_obs = FALSE,
    reassign_taxa = FALSE
...
fungi_asv <- readd(taxdata, cache = cache) %>%
  taxa::filter_taxa(
    taxon_names == "Fungi",
    reassign_obs = FALSE,
    reassign_taxa = FALSE
...
ecm_counts <- readd(taxon_reads, cache = cache) %>%
...
Figure \@ref(fig:correlog): 
  ...
  - Very similar results for different amplicons and technologies
  - Unifrac shows very little distance structure.
...
variog_data <- light_plan %>%
  ...
  mutate_at("correlog", lapply, readd, character_only = TRUE, cache = cache) %>%
  mutate_at("correlog", map, "mantel.res") %>%
  mutate_at("correlog", map_lgl, ~ min(.x[,"Pr(corrected)"], na.rm = TRUE) < 0.05) %>%
  ...
  mutate(
    variofit = ifelse(mantel_1, variofitST2, variofit2)
    ) %>%
  mutate_at(c("variofit", "variogST", "variofitST2", "variofit2"), map, readd, character_only = TRUE, cache = cache) %>%
  select(mantel_0, mantel_1, variofitST2, variofit2, variofit, variogST, guild, metric, tech, amplicon, algorithm) %>%
...
variog_empir %>%
  ...
  geom_line(data = filter(variog_fit, timelag == "1", algorithm == "PHYLO"), size = 0.5, color = "darkcyan", alpha = 0.8) +
  geom_line(data = filter(variog_fit, timelag == "0", algorithm == "PHYLO"), size = 0.5, color = "red1", alpha = 0.8) +
...
Figure \@ref(fig:spatio-temporal) and Table \@ref(tab:variofit):
 ...
 - Spatial ranges are all consistent, given the wide confidence intervals.
 - For most models the temporal aspect showed poor convergence.
 - Unifrac shows unconvincing distance structure. (No surprise that the model doesn't fit well)
 - similar results for whole community and ECM -- ECM dominate community.
