...
We therefore developed a new workflow to assemble ASVs from the long amplicons by splitting the reads into homologous domains (Figure\ \@ref(fig:regions)a), independently denoising each segment, concatenating the denoised domains, and finally forming consensus sequences based on ITS2 identity.
...
To assess the completeness of sequencing using each of the sequencing strategies, ASV and OTU accumulation curves were generated using the `iNEXT` package [@hsieh2016]. Additionally, average estimated ASV and OTU richness was calculated for each sample after rarefying to 100 sequences using 100 repetitions. ASV and OTU richness were then compared for each biological sample as sequenced by the different strategies. For each pair of strategies, the best fit slope for the ASV richness recovered by each strategy was calculated by total least squares using the `deming` package [@R-deming]. For this analysis, samples in the PacBio and Illumina datasets were combined bioinformatically to match the corresponding samples from the incompletely demultiplexed Ion Torrent dataset.

The effect of sequencing technology and amplicon length on the recovered fungal community composition, as assessed by the Bray-Curtis dissimilarity, was determined by PERMANOVA [@anderson2003] using the `adonis2` function in the `vegan` package [@R-vegan].
...
In contrast, extraction of only the ITS2 region before quality filtering (Figure \@ref(fig:qual-check)c) resulted in the loss of `r percent(1-readd(filtercounts_ITS2)['pb_500']/readd(rawcounts)['pb_500'], format = "d")` of trimmed long amplicon PacBio reads.
...
The length of the long amplicon reads varied widely, from `r filter(readd(reads_table), region == "long") %$% range(length) %>% paste(collapse = " to ")` bp, with a mean of `r filter(readd(reads_table), region == "long") %$% weighted.mean(length, reads) %>% round()` bp.
...
A larger fraction of ITS2 sequences extracted from the long amplicon dataset were shorter than 140 bp than from the short amplicon datasets (Figure \@ref(fig:ITS2-length)).
The taxonomic identity of these sequences will be discussed below.
...
ASV and OTU richness estimates after rarefaction were strongly correlated between the three sequencing technologies applied to the short amplicon library ($R^2 =$ `r dplyr::filter(dplyr::bind_rows(readd(demingfits_ASV), readd(demingfits_OTU)), endsWith(as.character(strategy_x), "Short"), endsWith(as.character(strategy_y), "Short"))$r_squared %>% range() %>% format(format = "f", digits = 2) %>% paste(collapse = " to ")`; Figure\ \@ref(fig:alpha-compare)).
...
Ion Torrent resulted in rarefied richness estimates which were `r dplyr::filter(dplyr::bind_rows(readd(demingfits_ASV), readd(demingfits_OTU)), startsWith(as.character(strategy_x), "Ion"), endsWith(as.character(strategy_y), "Short"))$value_x %>% divide_by(1, .) %>% subtract(1) %>% range() %>% percent(format = "f", digits = 0) %>% paste(collapse = " to ")` greater than the other technologies, an effect which is also visible in ASV (Figure\ \@ref(fig:accum-ASV)) and OTU (Figure\ \@ref(fig:accum-OTU)) accumulation curves.
Richness estimates were somewhat less strongly correlated between the PacBio long amplicon dataset and the three short amplicon datasets ($R^2 =$ `r dplyr::filter(dplyr::bind_rows(readd(demingfits_ASV), readd(demingfits_OTU)), endsWith(as.character(strategy_x), "Long") | endsWith(as.character(strategy_y), "Long"))$r_squared %>% range() %>% format(format = "f", digits = 2) %>% paste(collapse = " to ")`; Figure\ \@ref(fig:alpha-compare)).
...
```{r alpha-compare, fig.scap="(ref:alpha-compare-s)", fig.cap="(ref:alpha-compare-s) (ref:alpha-compare)", fig.height=8, fig.width = 4, out.width = "50%"}
ggpubr::ggarrange(
...
Both datasets from the short amplicon library showed no temporal autocorrelation, but a weak negative spatiotemporal correlation at 1\ m and 1\ year, which was probably a statistical artifact.
