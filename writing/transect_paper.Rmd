...
Forward and reverse reads were denoised using `DADA2` version `r packageVersion("dada2")` [@callahan2016] using seperate error models and then merged.
...
- Fraction where 5.8S was detected
- Length/ASV count/ASV fraction of extracted regions.
...
(ref:venn2) (*alternate to \@ref(fig:venn)*) Venn diagrams showing shared ITS2-based ASVs (left) and 97% OTUs (right) between different sequencing technologies from the same short amplicon library.
In each region, the number of ASVs/OTUs is given above, while the fractions of reads for each sequencing strategy are shown below.

```{r venn2, fig.cap = "(ref:venn2)", out.height = "30%", fig.show="hold"}
venndata(readd(asv_table), ASVs, cols = c("pb_483", "SH-2257", "is_057")) %>%
  vennplot_data(ASVs) %>% {
  plot(
     select(., set, ASVs) %>% deframe() %>% replace_na(0) %>% eulerr::venn(),
     legend = list(
...
venndata(readd(otu_table), OTUs, cols = c("pb_483", "SH-2257", "is_057")) %>%
  vennplot_data(OTUs) %>% {
  plot(
    select(., set, OTUs) %>% deframe() %>% replace_na(0) %>% eulerr::venn(),
    legend = FALSE,
...
(ref:venn3) (*alternate to \@ref(fig:venn)*) Venn diagrams showing shared ITS2-based ASVs (left) and 97% OTUs (right) between different amplicons.
In each region, the number of ASVs/OTUs is given above, while the fractions of reads for each amplicon are shown below.
Long amplicons were sequenced on PacBio RS II, short amplicons were sequenced on PacBio RS II, Illumina MiSeq, and Ion Torrent Ion S5.
ASV/OTU counts for short amplicons reflect detection by any of the three technologies, and read counts represent the mean fraction of reads across the three technologies.

```{r venn3, fig.cap = "(ref:venn2)", out.height = "30%", fig.show="hold"}
readd(asv_table) %>%
...
In all cases, the read counts for shared ASVs/OTUs were correlated ($R^2 = `r compR2("SH-2257", "is_057", "OTU")` to $).
...
In contrast, the length of the long amplicon reads varied widely, from `r filter(readd(reads_table), region == "long") %$% range(length) %>% paste(collapse = " to ")` bp, with a median of 
