...
<!-- R4: consider citing Kurtzman and Robnett, 1998 Antonie van Leeuwenhoek 73: 331 (and other
papers) who used the LSU region to ‘barcode’ yeasts -->
...
<!--R4: very true, great point. In fact, within-species ITS variation was also nicely discussed in
Lindner and Banik, 2011 Mycologia 103: 731 in case you want another citation-->
...
ASVs have been suggested to be less dataset specific that cluster-based OTUs [@callahan2017]. <!-- R5: cross out that -->
...
<!--R4: other literature you may want to consider citing: another phylogeny-based taxonomic
assignment method is SAP (statistical assignment package, Munch et al., 2008a Syst Biol 57: 750
and Munch et al., 2008b Phil Trans Roy Soc B 363: 3997), another lowest common ancestor
approach MEGAN (metagenomic analysis) Huson et al., 2016 PLoS Comp Biol 12: e1004957 that
can parse BLAST reports or RDP Classifier reports.Near L114 - More refs to consider: See Ashelford et al., 2005 Appl Env Microbiol 71: 7724,
...
Consequently <!--R5: add comma--> taxonomic assignments based on these databases may be incorrect or inconsistent.
...
The Unite database is a more curated <!--More curated?-->attempt to include all publically available ITS sequences (800\ 000 as of release 8.0), originally targetting fungi but now expanded to include all eukaryotes [@nilsson2019a], where efforts have been made to correct incorrect annotations and exclude low-quality sequences [@abarenkov2018a].
...
the Warcup ITS database, containing 18\ 000 manually curated fungal ITS sequences [@deshpande2016], and the RDP fungal LSU training set, containing 8000 manually curated LSU sequences from fungi and 3000 from other eukaryotic groups [@liu2012].
...
<!--R4: RDP reference sets include UNITE ITS and Warcup ITS as well as an LSU reference set
(Liu et al. 2012). I think it would be more clear to replace ...(Unite, Warcup, RDP) to (UNITE ITS,
Warcup ITS, Liu LSU)? As ‘RDP’ generally refers to the better-known prokaryote SSU rRNA gene
reference set.-->
...
Both sites were located in West Sudanian savannah woodlands [@olson2001;@yorou2014] dominated by the ECM host tree _Isoberlinia doka_ (Caesalpinioideae).
<!-- R5: This does not match the taxonomic info in the intro-->
...
For each sample, coarse organic debris was removed from the soil surface and a sample of approximately 5\ cm\ × 5\ cm\ × 5\ cm was extracted with a sterilized knife blade and homogenized. <!--R4: how was the knife sterilized, how were the samples homogenized? Preserved at what
...
<!--R5: -info on extraction techniques is missing here-->
...
The short amplicon (approximately 300\ bp) targeted the full ITS2 region as well as parts of the flanking 5.8S and large subunit (LSU) rDNA, using gITS7 [@ihrmark2012] as the forward primer and a mix of ITS4 [@white1990amplification] and ITS4a [@urbina2016] as the reverse primer.
<!--R4: “a mix of ITS4 and ITS4a” were equal mounts mixed? Please specify in text.-->
...
Each PCR run also included a blank sample and a positive control consisting of freshly extracted DNA from a commercially purchased fruitbody of *Agaricus bisporus*.<!--R5: In the future I strongly recommend using a positive control community of known taxa. this really helps to parameterize the bioinformatics to make sure you are capturing the approximate diversity that is expected based on this more complex control (rather than using one species, which is less helpful) -->
See Supplementary methods for PCR and library preparation protocols.
<!--I always recommend that full methods be in the main text, not in the supplement, it only
...
<!--R4: I suggest that you refer to a specific supplementary figure or table, instead of the whole
supplementary methods document here and elsewhere.-->
...
<!--R4: I suggest that the authors describe what the LSUx and tzara packages are used for if
these are new and being introducing for the first time here.-->
...
<!--R4: It would be nice if the colors used in part b corresponded with the same colors in part
a. What does 32S refer to here? Is this data based on rarefied, normalized data?-->
<!--R5: Not clear what 32S is here?-->
...
<!--R4: ‘Phylogenetic inference and TAXONOMIC assignment’-->
...
<!--R5: I realize that is covered in the Supp. methods in more detail but I would at least give one sentence about how you chose some reference sequences to parameterize the tree here-->
...
<!--R4: ...LSU region using RDP. I suggest you refer to the RDP LSU reference set (Liu et al., 2012)
instead of just to RDP which is better known for its prokaryote SSU rRNA gene classifier.-->
...
<!--R4: “refined using a phylogenetic tree” instead of ‘the’L275 - suggest you spend a few lines to describe what exactly what the phylotax package does if
it’s new and you’re introducing it here for the first time-->
...
<!--R4: a good citation is for the LCA algorithm is MEGAN Huson et al. 2016, see comment above-->
This algorithm has been used to assign a consensus taxonomy based on a list of top BLAST hits [LCAClassifier, @lanzen2012] or k-mer similarity scores [mothur's k-nearest neighbor method, @schloss2009], but here is used to resolve conflicts between assignments from different algorithms and databases.
...
<!--R4: ‘capture’ usually refers to the enrichment of metagenomic DNA fragments using bead
based selection for example. Here, consider something like ‘Reproducibility of sequence
detection using different sequencing technologies’-->
<!--R5: This has a specific meaning for many people that is about sequences obtained via “capture” using hybridization- i would use a different heading here to make it more obvious what you are referring to-->
...
In particular, the majority of the `r sum(readd(venn_ASV)[c("1001","0001"),"ASVs"])` unique Ion Torrent ASVs were found to be shared with other sequencing technologies upon OTU clustering.<!--R5: extra %-->
...
(ref:accum-ASV) Each curve represents rarefaction of a single sample or pooled pair of samples. Points at the end of each curve represent the actual read depth and observed ASV richness. Panel at left has enlarged scale to show PacBio more clearly.
<!--R4: If you normalized your dataset by rarefying your samples to the lowest library size, it
would be helpful if you added a vertical line to the plot indicating ASV richness after
Normalization.-->
<!--R5: again, were these analyses conducted on the entire (i.e. fungal and non-fungal) dataset, or just the fungal community?-->
...
<!--R4: Add a period after the figure title. change ‘ideal’ slope to ‘1:1 line’ or something
similar. Is this figure based on rarefied, normalized data? Indicate in the text.-->
...
(ref:taxon-chart) Fraction of ASVs (left) and reads (right) assigned to each taxonomic rank, for different sequencing technologies (PacBio RS II, Illumina MiSeq, Ion Torrent Ion S5), amplicons (Long, Short), reference databases (Unite, Warcup, RDP), and assignment algorithms (PHYLOTAX, Consensus, RDPC, SINTAX, IDTAXA).
...
<!--R4: This figure needs a ‘short’ title. Suggest you change RDP to RDP LSU training set. I’d
...
```{r taxon-chart, fig.cap = "(ref:taxon-chart)", fig.scap = "(ref:taxon-chart-s)", out.width = "100%"}
...
<!--R4: confidence intervals are not shown? If you’re doing to discuss CI then consider adding
them to your figure.-->
...
which we interpret as supporting the validity of abundance-based beta diversity measures in metabarcoding [but see @castano2020 and references therein].
<!--R4: ‘validity of abundance-based beta diversity measures’. Anyone can choose to treat their
data as if read proportion/relative proportion is informative, but for fungi with different thallus
sizes (yeast-like, filamentous), different nuclear states (haploid, diploid/dikaryons), abundant
sporulators, etc. what does this really mean? This is a very contentious statement and I’m not
sure that the data you have presented makes this case in any way. See Amend et al. 2010
Molec Ecol 19: 5555, Amend et al. 2010 PNAS 107: 13748 supp mat, Elbrecht and Leese, 2015
PLoS ONE 10: e0130324 for a non-fungal example, but interesting because like fungi,
arthropods come in ranges of body sizes that differ by orders of magnitude and they are multi-
cellular (unlike prokaryotes where relative abundance is often used, despite the fact that the
rRNA genes exist in differently sized tandem arrays that differ across lineages). Some studies
have even proposed ‘correction factors’ to make read counts more meaningful when assessing
species abundance. I think this statement needs to be re-written to be more conservative, as
your methods do not directly test whether read abundance reflects real species abundance.
You may even want to discuss and/or cite some of the literature that has directly tackled this
Issue.-->
<!--R5: These are all short amplicons, and previous studies have indicated strong bias in short amplicon HTS, across different sequencing technologies. The correlation alone cannot be used as evidence for the validity of abundance-based diversity measures. Especially as these R2 values are on the borderline of what is interpreted as "strong".-->
...
<!--R4: I’m uncertain how to interpret your rarefaction curves since both axes are on a log scale.
Does adequate read depth, saturated sampling, plateau in the same way that you would expect
curves to on regular axes as this is normally plotted? Please clarify in the main text.-->
...
We assert that, for studies targeting ECM fungi in particular, more complete capture of groups with high rDNA variability such as *Tulasnella* (and ideally other Cantharellales) is worth the read-depth spent on non-target groups.
...
<!-- R4: replace ‘RDP fungal training set’ with RDP LSU training set (Liu et al., 2012) as the RDP
has 3 fungal training sets (UNITE ITS, Warcup ITS, LSU) in addition to the better-known
prokaryote training set SSU rRNA.-->
...
<!--R5: This discussion of placement is good. However, it suffers from one serious weakness. It seems to assume that when things are placed that they are placed CORRECTLY. In my experience some things are obviously incorrectly placed. You should explicitly say whether you actually dealt with his problem or not. If not, at least give some caveats here!-->
...
<!--R4: in a ‘principled’ manner. Consider rephrasing to ‘uses phylogenetic relationships to
resolve disagreements.’ Do you need to set a bootstrap support cutoff to get a good phylotax
assignment? What cutoff did you use here? Does phylotax use neighbor joining or ML or
Bayesian tree construction? These details need to be added to your method section in the main
text. How does phylotax handle ambiguously aligned regions of ITS or LSU divergent domains?
Are they removed, recoded, ignored?-->
<!--R5: How were the results from this algorithm validated? On which taxonomic groups of fungi and/or non-fungi was it tested? How were the results of the other taxonomy methods checked for errors?-->
...
This corresponds to a space-for-time substitution rate of `r readd(variofit_table) %>% select(-variofit, -(std.error:conf.high)) %>% pivot_wider(names_from = term, values_from = estimate) %>% filter(timelag == 1, mantel == TRUE) %>% mutate(spacetime = range / timerange) %>% arrange(guild) %$% glue_collapse(format(spacetime, format = "f", digits = 2), " and ")`\ m/year for the total fungal community and ECM community, respecitvely.
...
Neither dataset from the short amplicon library showed significant temporal autocorrelatio.<!--R4: autocorrelation-->
