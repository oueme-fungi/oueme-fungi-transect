...
DNA concentrations after extraction and PCR, as well as sequencing reads for PacBio and Illumina, are shown per sample in Supplementary Figure \@ref(fig:sample_depth).
...
(ref:region-asv) Plot of total number of ASVs recovered vs. fraction of demultiplexed reads successfully mapped to ASVs for different rDNA regions extracted from long PacBio amplicons using `LSUx`. Some regions were formed by concatenation of extracted regions, as follows: LSU = LSU1--D1--LSU2--D2--LSU3--D3--LSU4; ITS = ITS1--5.8S--ITS2; 32S = 5.8S--ITS2--LSU; long = ITS--LSU. For this figure, subregions were concatenated prior to generating ASVs with `DADA2`.
...
Red nodes/edges represent increased read/ASV count for the short amplicon library, while teal nodes/edges represent increased read/ASV count for the long amplicon library.
Values are log~10~ of the ratio of mean values across all datasets (sequencing technology, assignment algorithm, reference database) for each amplicon.
