...
Fungal metabarcoding studies using short-read HTS technologies such as 454, Illumina, and Ion Torrent
...
Using BLAST requires *a priori* choice of thresholds for different taxonomic ranks.
Although GenBank is often used as a first step for sequence identification, the database is ...
Mention Unite, RDP, Warcup.
<!-- R3: I would like to see a few more sentences of discussion of the issues with taxonomic assignments … I’m sure you know of them and are being brief for word limits…but I think it would be useful to mention more for young readers or people unfamiliar with fungi and issues you can come up against...like huge percentages of your sequences not getting names due to poor databases, multiple sequences getting the same names, multiple ITS copies within the same species getting different names, chimeric or incorrect taxonomic identifications in BLAST,  etc etc-->
...
<!-- R1: Palmer et al.(2018) use a similar ‘global’ approach to taxonomic assignment in the amptk pipeline that I think would be worth citing here. (They also discuss length variation in the ITS as a factor influencing taxa recovery, which is relevant to the points highlighted in the previous paragraph). -->
...
<!-- R2: This comes as a surprise - why use multiple sequencing strategies? There is no background given on the strategies used. Why use three different short-read platforms? Some discussion of how error rates differ and how the proprietary software (e.g., PacBio, Illumina) differs in its way of scoring is needed. The overall approach needs more justification in the Introduction section. I suggest either framing the question more in terms of using 1 vs 2 rRNA regions; although it must be placed in context based on the fact that other studies have used multiple rRNA regions in long-read studies); perhaps exploring in more detail how read depth affects the conclusions; however, read depth has not been mentioned in the Intro at all.-->
...
<!-- R2: The need for new algorithms is not introduced or established in the Introduction. For example, what are the limitation of ITSx?-->
...
<!-- R3: I entered those locations into google maps. The first one took me to Ghana, the second took me to the Pacific Ocean. Something is off – you need to fix this.-->
...
<!-- R3: this is confusing. Do you mean that you had one 25m transect and 25 samples were taken along that one transect at 1m intervals?-->
...
<!-- R3: why would you lyse in the field? Aren’t you concerned about needing to refrigerate or freeze your sample? This is highly unorthodox-->
...
Sequencing results for these samples differed significantly (PERMANOVA with 9999 permutations, $p < `r readd(buffer_adonis)["buffer","p.value"] %>% formatC(format = "f")`$, $R^2 = `r readd(buffer_adonis)["buffer","R2"] %>% formatC(format = "f", digits = 2)`$) from samples preserved using the Xpedition^TM^ lysis buffer (Figures \@ref(fig:buffer-pcoa), \@ref(fig:buffer-compare-long), and \@ref(fig:buffer-compare-short)); as such these samples were excluded from our spatial analyses.
<!-- R3: which samples were excluded? The ones that were hand lysed or the ones that were preserved in lifeguard?-->
...
<!-- R3: so the gITS7 primer has a barcode and adaptor on it? I’m having a bit of trouble following your methods-->
...
<!-- R3: were technical replicates pooled prior to bead cleaning? Personally, I do not think technical replicates are necessary when using NGS (see Smith and Peay Plos One 2014) but I still need to be able to clearly follow your methods-->
...
<!-- R3: not sure that this line is necessary-->
...
<!-- R3: Note: I have heard from many people that DADA2 performs poorly for ITS and that it was designed for 16S-->
...
<!-- R3: you spell out RDP and cite it here, but not in your first mention of it at line 109. I would switch this around.-->
...
Code to extract the regions, including annotated seed alignments and CMs, is available in the new R package `LSUx`.
<!-- R3: I tried to download LSUx but got an error. Can you do something about this? 

> install.packages("LSUx")
And it threw the error
“Warning message:
package ‘LSUx’ is not available (for R version 4.0.2)”

is this the github repo? https://rdrr.io/github/brendanf/LSUx/src/R/lsux.R can you put a link to this instead or in addition?

If this is a methods paper, is the point to introduce LSUx? Can you provide a tutorial or better explanations of how one might actually implement LSUx? I have run analyses in QIIME2 and UPARSE many times but I don’t think I could figure out how to run your pipeline based on your paper here, and isn’t the point of this paper to make a new method available to your readers?-->
...
<!-- R3: similar to issue with RDP, you are stating amplicon sequencing variants (ASVs) here but introduced it for the first time on line 70. -->
...
<!-- R3: in general, I’m struggling to follow a lot of these methods and do not think that I could replicate your analyses -->
...
<!-- R3: please explain how figure S5 shows me that ITS2 is the most variable of the amplified regions. I don’t get it.-->
...
The process of assigning consensus full-length ASVs was carried out using the new `tzara` package for R.
...
<!-- R3: I’m going to stop pointing this out – but state the full name (e.g. Ribosomal Data Project) the first time you introduce something, then in every other instance refer to it by its abbreviation (e.g. RDP).-->
...
<!-- R3: why were these 3 algorithms chosen?-->
...
<!-- R3: why would you include at any level of confidence and not limit yourself to probable or highly probable?-->
...
<!-- R3: But didn’t you use fungal specific primers? Wouldn’t the non-fungal kingdom reads just be PhiX?-->
...
<!-- R2: What is the justification for trusting the (fast, rough) tree here compared to the other algorithms?-->
...
<!-- R2: I am not a permanova expert but is it allowed when there is some problem of independence here because PacBio RS II includes both short and long reads?-->
...
<!-- Add citations for B-C and Unifrac-->
Each of these distance matrices was used to calculate a Mantel correlogram for distances of 0--12\ m.
<!-- What is the justification for 0-12 m categories? Why not use 0-1 m? -->
...
Demultiplexing, primer trimming, and quality filtering reduced these totals by `r (1-readd(filtercounts_full)/readd(rawcounts))["pb_500"] %>% percent(format = "d")` for PacBio long reads, but only by `r (1-readd(filtercounts_full)/readd(rawcounts))["pb_483"] %>% percent(format = "d")` for PacBio short reads, resulting in a similar number of filtered reads for the two strategies.
...
The abundance of the second ASV was `r readd(agaricus_reads) %>% as.data.frame() %>% {.[["f2ab49e8"]]/.[["39d80c19"]]} %>% keep(~. > 0) %>% percent(format = "f", digits = 1) %>% paste(collapse = " and ")` that of the primary *A. bisporus* ASV in the two Illumina positive controls.
...
# Data Accessibility {-}
 ...
 - Sequence data, including raw reads and ASV sequences, will be deposited at the European Nucleotide Archive (ENA) prior to final publication.
...
These packages are currently being prepared for submission to Bioconductor.
If they are not accepted at Bioconductor prior to final publication, they will be archived at Dryad.
