...
agaricus_reads <- readd(proto_physeq, cache = cache) %>%
  phyloseq::subset_samples(sample_type == "Pos" | (seq_run == "is_057" & well == "E7")) %>%
  prune_taxa(taxa = agaricus) %>%
  prune_taxa(taxa = colSums(otu_table(.)) > 0) %>%
  otu_table()

pos_control <- agaricus_reads %>% as.matrix() %>% colSums() %>% which.max() %>% names()

pos_control_reads <- readd(proto_physeq, cache = cache) %>%
  phyloseq::subset_samples(sample_type != "Pos" & !(seq_run == "is_057" & well == "E7")) %>%
  prune_taxa(taxa = pos_control) %>%
  otu_table() %>%
  rowSums()
all_reads <- readd(proto_physeq, cache = cache) %>%
  phyloseq::subset_samples(sample_type != "Pos" & !(seq_run == "is_057" & well == "E7")) %>%
  otu_table() %>%
  as.matrix() %>%
  rowSums()
...
The abundance of the second ASV was `r agaricus_reads %>% as.data.frame() %>% with(f2ab49e8/`39d80c19`) %>% keep(~. > 0) %>% percent(format = "f", digits = 2) %>% paste(collapse = " and ")` that of the primary ASV in the two Illumina positive controls.
The consistency of this ratio suggests that it represents true inter-copy variation within the specimen, rather than sequencing or PCR error.
