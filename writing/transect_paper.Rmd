...
The Unite database is an attempt to include all publically available high-quality ITS sequences (800\ 000 as of release 8.0), originally limited to fungi but now expanded to include all eukaryotes [@nilsson2019a], where efforts have been made to correct incorrect annotations and exclude low-quality sequences [@abarenkov2018a].
...
A subsample of approximately 250\ mg total of soil was collected from two locations in the homogenized soil sample and stored in DNA-preservation buffer before return to the laboratory for extraction (see Supplementary Methods \@ref(Sampling-supplement) for preservation and extraction methods).
...
We then independently denoised reads from each domain and concatenating the denoised domains for each read.
Finally denoised reads were clustered based on 100% ITS2 identity, and a consensus ASV was calculated for each cluster.
...
The PHYLOTAX algorithm resolves conflicts among one or more primary assignment methods (see Figure\ \@ref(fig:phylotax) and Supplementary Methods \@ref(phylotax)).
