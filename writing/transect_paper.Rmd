...
variog_data %>%
  ...
  pmap_dfr(
    ...
    function(mantel, variofit, term, ...) {
      if (mantel) {
        tryCatch(broom::confint_tidy(object = variofit, parm = term) %>% as.list %>% as.data.frame,
                 error = function(e) tibble(conf.low = NA_real_, conf.high = NA_real_))
      } else {
        tibble(conf.low = NA_real_, conf.high = NA_real_)
      }
    })
  ) %>%
