...
### Denoising long-reads: the numbers are against us
...
In order to bypass this limitation, we implemented a workflow which divides the long rDNA amplicon into homologous domains (R package `LSUx`), denoises the domains individually using DADA2, and then recombines the denoised domains to form full-length denoised amplicons (R package `tzara`).
...
Assignments were considered as reliable if at least five of the six remaining primary assignment methods (2 databases × 3 algorithms) resulted in the same kingdom-level assignment, with no conflicting assignments.
Two sequences belonging to *Tulasnella*, which is known to have an accelerated rate of molecular evolution in the rDNA and often forms long, unstable branches in phylogenies [@moncalvo2006], were excluded at this step.
