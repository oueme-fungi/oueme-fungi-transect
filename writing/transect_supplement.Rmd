...
<!-- R3: your caption isn’t in line with your axes. Your axes say MDS1 and MDS2 – so it looks like a NMDS to me. But you caption says PCA. There is also insufficient information to jusdge this figure - No information on if reads were normalized or not, or what beta diversity metric this is based on, etc-->
...
<!-- Is there an error in the legend? It appears that node 3 is assigned to Taxa 1 based on box color coding and node 4 to Taxon 2 based on different box color coding. In the legend, it states that node 3 is assigned to Taxa 1, but there are three matches to Taxon 2 at that node and Taxon 2 is presented in Salmon color consistent with the box color for node 3. As such, I think the authors meant to designate node 3 as being assigned to Taxon 2. If this is wrong, then more details in the legend explaining why the greater presence of matches to Tax2 yet assignment to Tax1 is needed. -->
...
(ref:qual-check-s) Read quality profiles for different sequencing strategies

(ref:qual-check) **(a)** per-base error rate; **(b)** expected number of errors per read, with threshold of 3 expected errors marked as a dashed vertical line. Statistics are based on quality scores assigned by technology-specific basecallers.

```{r qual-check, fig.cap = "(ref:qual-check-s). (ref:qual-check)", fig.scap = "(ref:qual-check-s)",fig.height=6}
ggpubr::ggarrange(
  readd(erate_plot),
  readd(eexp_plot),
