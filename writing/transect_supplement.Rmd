...
For each sample, any coarse organic debris <!--Do you have any definition? Wider than 5 mm? MR-->was removed from the soil surface and a sample of approximately 5\ cm\ × 5\ cm\ × 5\ cm was extracted with a sterilized knife blade.
...
## DNA extraction, amplification, and sequencing {#DNA-supplement}
...
DNA was quantified fluorometrically using Quant-iT^TM^ PicoGreen^TM^ dsDNA (Thermo Fisher Scientific, Waltham, MA, USA) fluorescent indicator dye on a Infinite F200 plate spectrofluorometer (Tecan Trading AG, Männedorf, Switzerland) according to the manufacturer's protocol.
...
### Uniform taxonomic classification for Unite, Warcup, and RDP
...
### Refining taxonomic assignments with a phylogenetic tree: `PHYLOTAX`
...
```{r buffer-compare-long, fig.scap = "(ref:buffer-compare-long-s)", fig.cap = "(ref:buffer-compare-long-s). (ref:buffer-compare-long)", fig.width = 10, fig.height = 10, out.width = "100%", cache = TRUE, extra.cache = gethash("buffer_compare_long")}
...
```{r buffer-compare-short, fig.scap="(ref:buffer-compare-short-s)", fig.cap = "(ref:buffer-compare-short-s). (ref:buffer-compare-short)", fig.width = 10, fig.height = 10, out.width = "100%", cache = TRUE, extra.cache = gethash("buffer_compare_short")}
oldtheme <- theme_update(panel.border = element_blank())
...
```{r sample-depth, fig.cap = "(ref:sample-depth)", fig.scap = "(ref:sample-depth-s)", fig.env = "sidewaysfigure", fig.width = 8, fig.height = 5, out.width = "100%", cache = TRUE, extra.cache = gethash("conc_plot", "reads_plot"), results='asis'}
...
(ref:read-compare-asv) Comparison between ASV read abundances for different sequencing strategies

(ref:read-compare) which were detected by one sequencing strategy but not the other are plotted as tick marks along the axes. Dashed line represents a constant ratio of read numbers. The blue line is a LOESS smooth of the data, with associated uncertainty in grey shading. $R^2$ value displayed is for log-transformed non-zero read numbers.

```{r read-compare-asv, fig.cap="(ref:read-compare-asv). ASVs (ref:read-compare)", fig.scap="(ref:read-compare-asv)", cache=TRUE, extra.cache=gethash("multi_table","comparisons")}
grid::grid.newpage()
grid::grid.draw(
  read_comparison(
    readd(multi_table),
...
(ref:read-compare-otu) Comparison between OTU read abundances for different sequencing strategies

```{r read-compare-otu, fig.cap="(ref:read-compare-otu). OTUs (ref:read-compare)", fig.scap="(ref:read-compare-otu)", cache=TRUE, extra.cache=gethash("multi_table","comparisons")}
grid::grid.newpage()
grid::grid.draw(
  read_comparison(
    readd(multi_table),
...
(ref:accum-OTU-s) OTU accumulation curves

(ref:accum-OTU) Each curve represents rarefaction of a single sample or pooled pair of samples. Points at the end of each curve represent the actual read depth and observed OTU richness. Note logarithmic axes.
...
```{r taxa, fig.cap = "(ref:taxa-s). (ref:taxa)", fig.scap="(ref:taxa-s)", fig.width = 8, fig.height = 5, out.width="100%", fig.env = "sidewaysfigure", cache=TRUE, extra.cache=gethash("heattree")}
...
```{r ecm, fig.cap = "(ref:ecm-s). (ref:ecm)", fig.scap = "(ref:ecm-s)", fig.width = 7, cache=TRUE, extra.cache=gethash("taxon_reads"), more.cache = datasets}
...
readd(venn_ASV) %>%
venn_table(ASVs, caption = "(ref:venn-table-asv-s). (ref:venn-table-asv)", caption.short = "(ref:venn-table-asv-s)", table.envir = "sidewaystable")
```
...
readd(venn_OTU) %>%
venn_table(OTUs, caption = "(ref:venn-table-otu-s). (ref:venn-table-otu)", caption.short = "(ref:venn-table-otu-s)", table.envir = "sidewaystable")
```
...
```{r variofit, results="asis", cache=TRUE, extra.cache=gethash("variofit_table")}
 readd(variofit_table) %>%
  ...
  kable(booktabs = TRUE, caption = "(ref:variofit)", linesep = c(rep("", 5), "\\addlinespace"), table.envir = "sidewaystable")
```
