...
R = cd $(<D) && echo $^ | Rscript $(ROPT) $(<F) $(RARGS) &>"$(patsubst %.R,%.Rout,$@.$(<F))"
...
%.dada.dadamap.rds %.dada.seqtable.rds %.dada.nochim.rds : split_rdata.R %.dada.Rdata 
	$(R)
