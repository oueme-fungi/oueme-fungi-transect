...
ion.otu <- read_tsv(iontorrent.otu.file) %>%
  mutate(n_reads = rowSums(select_if(., is_numeric))) %>%
  left_join(read_tsv(iontorrent.classification.file)) %>%
  select(-matches("[A-H]\\d+"), -contains("ITS")) %>%
  left_join(read_tsv(iontorrent.funguild.file) %>% rename(OTU = Group)) %>%
  select(-matches("[A-H]\\d+"), -contains("ITS")) %>%
  rename(Kingdom = Domain) %>%
  dplyr::mutate_at(dplyr::vars(Phylum:Species), dplyr::na_if, "?") %>%
  dplyr::mutate_at(dplyr::vars(Phylum:Species), dplyr::na_if, "unidentified") %>%
  ...
  # Add the representative sequence for each OTU and calculate the length
  left_join(
    seqinr::read.fasta(file = iontorrent.repseq.file, as.string = TRUE) %>%
    { tibble(OTU = names(.),
             RepSeq = unlist(.),
             SeqLen = str_length(RepSeq)) }) %>%
  
  # Remove the positive control
  filter(is.na(Species) | Species != "Agaricus bisporus") %>%
  
  # Keep the data in this order.
  mutate_at(dplyr::vars(Kingdom:Species),
            ~ factor(.) %>% fct_inorder)
  

ion.platemap <- read_tsv(iontorrent.otu.file) %>%
  column_to_rownames("OTU") %>%
  as.matrix %>%
  colSums %>%
  tibble(Well = names(.), n_reads = .) %>%
  mutate_at("Well", plyr::mapvalues, well_key$Well, well_key$True_Well, FALSE) %>%
  full_join(plate %>%
              select(Plate, Well, sample_type) %>%
              full_join(
                expand.grid(Plate = 1:2,
                            Row = LETTERS[1:8],
                            Column = 1:12) %>%
                  tidyr::unite(Well, Row, Column, sep = ""))) %>%
  mutate(Plate = "IonTorrent") %>%
  mutate_at("sample_type", fct_explicit_na, "None") %>%
  group_by_if(~ !is.factor(.)) %>%
  summarize(sample_type = paste(as.character(sort(unique(sample_type))), collapse = "/")) %>%
  ungroup %>%
